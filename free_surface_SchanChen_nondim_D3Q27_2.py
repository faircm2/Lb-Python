...
while iteration < TOTAL_ITERATIONS:
    ...
    while max_eps > epsilon_threshold and ph_iter < max_ph_iters:
        p, h = ph(h, rho, u_ckl_star, iteration)
        delta_p = p - _p0
        p = _p0 + omega_sor * delta_p
        epsilon = np.abs(delta_p) / np.maximum(rho, 0.1)
        max_eps = np.max(epsilon)
        _p0 = p
        ph_iter += 1
        ...
        # --- STALL DETECTION + PERTURBATION ---
        if len(eps_hist) == eps_hist.maxlen:
            rel_spread = (max(eps_hist) - min(eps_hist)) / max(np.mean(eps_hist), 1e-12)
            if rel_spread < epsilon_threshold:
                debug_log('ITER', 'Stalled (rel_spread=%.6e), perturbing...', rel_spread)
                noise = 1e-8 * (2 * np.random.random(_p0.shape) - 1)
                _p0 += noise * np.maximum(rho, 0.1)
                epsilon = np.full_like(rho, epsilon0)
                max_eps = epsilon0
                eps_hist.clear()
                eps_hist.append(max_eps)
            elif ph_iter > 100 and max_eps > 1e-3:
                debug_log('ITER', 'High eps plateau (%.3e), perturbing...', max_eps)
                noise = 1e-7 * (2 * np.random.random(_p0.shape) - 1)
                _p0 += noise * np.maximum(rho, 0.1)
                epsilon = np.full_like(rho, epsilon0)
                max_eps = epsilon0
                eps_hist.clear()
        ...
        if ph_iter % 10 == 0:
            debug_log('ITER', 'Simulation Execution -> iteration %d; ph_iter %d; max_eps=%.6e', iteration, ph_iter, max_eps)
    ...
    if max_eps > epsilon_threshold:
        debug_log('ITER', 'Simulation Execution -> ph stalled at global %d, eps=%.3e', iteration, max_eps)
    #################################################################################################################################
