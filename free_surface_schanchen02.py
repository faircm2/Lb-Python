...
def c_second_derivative(lamda, dx=1.0, dy=1.0):
    ...
    for k in range(1,9):
        ...
        sum_lambda += lamda_shift
        #sum_xx = c[k, 0] * lamda_shift
        #sum_yy = c[k, 0] * lamda_shift

    laplacian = (sum_lambda - 14 * lamda)/(5*dx)
...
def ph(hn, _rho, u_ckl_star, dx=1.0, dy=1.0):
    ...
    collision = hn - tau_exp * (hn - E_p) - (1/3) * E_exp * div_u * dx
...
def gi(_gi, _gi_c, u_ckl, rho, mu):
    du_dx_x, du_dy_x = c_first_derivative(u_ckl[0,:,:], dx, dy)
    du_dx_y, du_dy_y = c_first_derivative(u_ckl[1,:,:], dx, dy)
    ...
    div_sigma_x = (np.roll(sigma_xx, -1, axis=1) - np.roll(sigma_xx, 1, axis=1) + 
                   np.roll(sigma_xy, -1, axis=0) - np.roll(sigma_xy, 1, axis=0)) / (2 * dx)
    div_sigma_y = (np.roll(sigma_yx, -1, axis=1) - np.roll(sigma_yx, 1, axis=1) + 
                   np.roll(sigma_yy, -1, axis=0) - np.roll(sigma_yy, 1, axis=0)) / (2 * dx)
    ...
    for i in range(9):
        ...
        deriv_term_0 = 3 * E[i] * c[i,0] / rho * d_dy * dx
        deriv_term_1 = 3 * E[i] * c[i,0] / rho * viscous_force_term * dx
...
plot_u_ckl_bounds(u_bounds, "u_ckl_bounds_ShanChen")
