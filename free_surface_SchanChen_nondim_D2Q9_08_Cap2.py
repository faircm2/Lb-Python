...
class FlowConfig:
    ...
    def g_x(self) -> float:
        ...
        return self.g * np.sin(self.alpha_rad)

    @property
    def g_y(self) -> float:
        """y-component of gravitational acceleration."""
        return -self.g * np.cos(self.alpha_rad)
...
def st_mu_phi(fc, _phi):
    ...
    """
    _mu_phi = 4 * st_beta(fc) * _phi*(_phi - 1) * (_phi - 0.5)  -  st_kappa(fc) * c_laplacian(_phi)
    return _mu_phi


def surfaceTensionForces(iteration, fc, _phi):
    ...
    _phi[0, 1:Yn+1]  = _phi_w_left   # current x=0
    _phi[1, 1:Yn+1]   = _phi_w_left     # left

    # Right wall
    _phi[Xn+1, 1:Yn+1] = _phi_w_right  # current x=Xn+1  
    _phi[Xn, 1:Yn+1]  = _phi_w_right    # right      
    ...
    # ──────────────────────────────────────────────────────────────
    # Top boundary: neutral gas (open channel) + strong 1-node pure-gas pinning
    # ──────────────────────────────────────────────────────────────
    # Set entire top ghost row to pure gas value (neutral wetting, open top)
    _phi[:, Yn+1] = fc.phi_star_G

    # Force pure gas at exact top corners and next-to-corner nodes
    # This ensures 1-node-wide tapering with no mixture at y=Yn
    _phi[0, Yn+1]   = fc.phi_star_G          # pure gas — left corner
    _phi[1, Yn+1]   = fc.phi_star_G          # pure gas — next left node (force 1-node taper)
    _phi[Xn+1, Yn+1] = fc.phi_star_G         # pure gas — right corner
    _phi[Xn, Yn+1]  = fc.phi_star_G          # pure gas — next right node (force 1-node taper)
    ...
    # ──────────────────────────────────────────────────────────────
    # Side fluid nodes: height-dependent overwrite for tapering
    # ──────────────────────────────────────────────────────────────
    taper_height = 2  # top 2 rows for taper to 1 node

    # Calculate the end index for the lower (full wetting) part
    lower_end = Yn + 1 - taper_height  # this is the number of nodes to keep full wetting

    # Overwrite lower part for strong climbing
    # Use correct slices: both sides now match exactly
    # Start from index 0 on _phi[1, ...] to align with _phi_w_left[0:...]
    _phi[1, 0:lower_end] = _phi_w_left[0:lower_end]     # length = lower_end
    _phi[Xn, 0:lower_end] = _phi_w_right[0:lower_end]   # length = lower_end

    # Force top taper rows on sides to pure gas (taper to 1 node or zero)
    _phi[1, lower_end:Yn+1] = fc.phi_star_G
    _phi[Xn, lower_end:Yn+1] = fc.phi_star_G
    # ──────────────────────────────────────────────────────────────
    ...
    inamuro_surface_tension_force = mu_c(fc, _phi) * c_first_derivative(_phi) #/ fc.CF
    # surface_tension_force Zhang eq(5) using chemical potential acc. Zhang eq(6) mu_phi
    # scale with division by  / fc.CF analog """Body force vector scaled to lattice units."""   
    zhang_surface_tension_force = st_mu_phi(fc, _phi) * c_first_derivative(_phi) #/ fc.CF

    return inamuro_surface_tension_force, zhang_surface_tension_force
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    inamuro_surface_tension_force, zhang_surface_tension_force = surfaceTensionForces(iteration, fc, _phi)
    ...
    if iteration == 0 or iteration==(fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0:
        if ADD_METRICS:
            ...
            SurfaceTensionForceCollector.append((iteration, _body_force, _InamuroSurfaceTensionForce, _ZhangSurfaceTensionForce))        
...
if ADD_METRICS: 
    ...
    series_labels = ["Body Force", "Inamuro Surface-Tension Force", "Zhang Surface-Tension Force"]
