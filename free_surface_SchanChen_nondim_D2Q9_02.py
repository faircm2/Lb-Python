...
def gradient_p0(p, n_dx=1.0, n_dy=1.0, n_dz=1.0):
    """
    Compute ∇p using c_first_derivative_d3q27.
    p: pressure field (Nx, Ny, Nz)
    Returns: grad_p (3, Nx, Ny, Nz) → [dp/dx, dp/dy, dp/dz]
    """
    dp_dx, dp_dy, dp_dz = c_first_derivative(p, n_dx, n_dy, n_dz)
    return np.stack([dp_dx, dp_dy, dp_dz], axis=0)  # ← (3, Nx, Ny, Nz)
...
def c_first_derivative0(lamda, n_dx=1.0, n_dy=1.0, n_dz=1.0):
    """
    D3Q27 analog of Inamuro Eq. (12)
    Uses only 6 face neighbors (|c|=1) → like D2Q9 uses 8
    """
    if lamda.ndim != 3:
        raise ValueError(f"Expected 3D lamda, got {lamda.shape}")

    # Use only |c|=1: indices 1 to 6
    neigh_idx = slice(1, 7)
    cx, cy, cz = c[neigh_idx, 0], c[neigh_idx, 1], c[neigh_idx, 2]

    neighbors = np.stack([
        np.roll(np.roll(np.roll(lamda, cx[i], 0), cy[i], 1), cz[i], 2)
        for i in range(6)
    ], axis=0)  # (6, Nx, Ny, Nz)

    scale = 1.0 / (10.0)  # same as 2D

    d_dx = np.sum(cx[:, None, None, None] * neighbors, axis=0) * scale / n_dx
    d_dy = np.sum(cy[:, None, None, None] * neighbors, axis=0) * scale / n_dy
    d_dz = np.sum(cz[:, None, None, None] * neighbors, axis=0) * scale / n_dz
...
def c_second_derivative0(lamda, n_dx=1.0, n_dy=1.0, n_dz=1.0):
    if lamda.ndim != 3:
        raise ValueError(f"Expected 3D lamda, got {lamda.shape}")
    ...
    sum_n = np.sum(neighbors, axis=0)
    scale = (6.0 / 12.0) / 5.0  # 6 neighbors, like 8/14 in 2D → 6/12
    d2x = scale * (sum_n - 6.0 * lamda) / n_dx
    d2y = scale * (sum_n - 6.0 * lamda) / n_dy
    d2z = scale * (sum_n - 6.0 * lamda) / n_dz
...
def gi0(_gi, _gi_c, u_ckl, rho, mu, iteration):
    """
...
while iteration < TOTAL_ITERATIONS:
    ...
    while np.max(epsilon) > epsilon_cutoff and ph_iter < max_ph_iters:
        ...
        eps_hist.append(np.max(epsilon))
        ...
        if ph_iter % 2 == 0:
            debug_log('ITER', 'Simulation Execution -> iteration %d; ph_iter %d; max_eps=%.6e : epsilon_cutoff=%.6e', iteration, ph_iter, np.max(epsilon), epsilon_cutoff)

    if np.max(epsilon) > epsilon_cutoff:
        debug_log('ITER', 'Simulation Execution -> ph stalled at global %d, eps=%.3e, using approx p', iteration, np.max(epsilon))
        # Fallback: smooth p to break stall
        from scipy.ndimage import gaussian_filter
        _p0 = gaussian_filter(_p0, sigma=1.0)  # Light smoothing 

    debug_log('ITER', 'Simulation Execution -> P-Corr.: iteration %d; ph_iter %d; max_eps=%.3e : epsilon_cutoff=%.3e', iteration, ph_iter, np.max(epsilon), epsilon_cutoff)
