...
while iteration < TOTAL_ITERATIONS:
    ...
    while np.max(epsilon) > epsilon_cutoff and ph_iter < max_ph_iters:
        ...
        ph_iter += 1
        if ph_iter % 1 == 0:
            debug_log('ITER', 'Simulation Execution -> iteration %d; ph_iter %d; max_eps=%.3e : epsilon_cutoff=%.3e', iteration, ph_iter, np.max(epsilon), epsilon_cutoff)
    ...
    debug_log('H', 'FINAL P-Corr.: iteration %d; ph_iter %d; max_eps=%.3e : epsilon_cutoff=%.3e', iteration, ph_iter, np.max(epsilon), epsilon_cutoff)
