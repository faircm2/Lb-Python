...
# Loop over L (1 to 100, step 10)
for L in {1..100..10}; do
    # Calculate Xn
    Xn=$(( 96 * 50 * L ))
    ...
    mpirun -n ${PROCESSES} --report-bindings python3 ./${PYFILE} >> "output_nodes_${NODES}_L_${L}.log" 2>> "error_nodes_${NODES}_L_${L}.log"
    if [[ $? -ne 0 ]]; then
