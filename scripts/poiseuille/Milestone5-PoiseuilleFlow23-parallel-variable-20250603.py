...
while iteration < TOTAL_ITERATION:
    ...
    # 7. Store only the last u_ckl (no need for list_avg_velocities for multiple iterations)
...
# Corrected function to collect the last u_ckl for the velocity map
def collect_u_ckl_for_velocity_map(nx, __u_ckl):
    _sendBuf_u_ckl = np.ascontiguousarray(__u_ckl[0, 1:-1, 1:Yn+1], dtype=np.float32)
    
    if rank == 0:
        _u_ckl_velocity_map = np.zeros((Xn, Yn), dtype=np.float32)
        _u_ckl_velocity_map = np.ascontiguousarray(_u_ckl_velocity_map, dtype=np.float32)
        
        counts = np.array([Xn//size] * size, dtype=np.int32)
        if size > 1:
            counts[-1] = Xn - (Xn//size) * (size-1)
        counts = counts * Yn
        ...
        comm.Gatherv(_sendBuf_u_ckl, [_u_ckl_velocity_map, counts, displacements, MPI.FLOAT], root=0)
    else:
        comm.Gatherv(_sendBuf_u_ckl, None, root=0)
    
    _u_ckl_velocity_map = comm.bcast(_u_ckl_velocity_map, root=0)
    return _u_ckl_velocity_map
...
if rank == 0:
    ...
    # Row 0, Col 1: Velocity map (updated to use _u_ckl_velocity_map)
    velocity_map(ax[0, 1], _u_ckl_velocity_map)
