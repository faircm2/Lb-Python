...
def bounceBackTopBottom(_ltc, _ltc_pre, _index_mapping_top, _index_mapping_bottom):
    ...
    for i, swap_needed in enumerate(_index_mapping_top):
        if swap_needed:
            _ltc[channel_indices[i], upper_boundary, :] = _ltc_pre[antichannel_indices[i], upper_boundary, :]

    boundary_values_top_pre_ltc.append([index, _ltc_pre[:, upper_boundary, :2]])
    boundary_values_top_ltc.append([index, _ltc[:, upper_boundary, :2]])

    
    # swaps on the bottom boundary
    for i, swap_needed in enumerate(_index_mapping_bottom):
        if swap_needed:
            _ltc[channel_indices[i], lower_boundary, :] = _ltc_pre[antichannel_indices[i], lower_boundary, :]

    return _ltc


#calulate boundary nodes X(0) and X(N+1) for periodic BC with presssure difference
def calcPeriodicBC(_roh_N, _u_avg_N, _roh_inlet, _roh_1, _u_avg_1, _roh_outlet, _ltc): 
    #N==Nx+1
    #1==1
    #X(0) = X(0)
    #X(N+1) = X(Nx+2)
...
streamed_lattice = f_eq_slc(roh, u_avg)
...
num_nodes = int(D/dy) + 1  # Number of nodes in inlet/outlet
...
# Initial plots
roh_inlet_plot_0, = ax[0].plot([], [], label="roh_inlet[0]", color="green")
roh_inlet_plot_1, = ax[1].plot([], [], label="roh_inlet[1]", color="blue")
velocity_inlet_plot_0, = ax[2].plot([], [], label="Inlet Velocity[0]", color="red")
velocity_inlet_plot_1, = ax[3].plot([], [], label="Inlet Velocity[1]", color="purple")

# Set titles, labels, and legends
ax[0].set_title("Inlet Density Profile [0]")
ax[0].set_xlabel("Node Index")
ax[0].set_ylabel("Density")
ax[0].legend()

ax[1].set_title("Inlet Density Profile [1]")
ax[1].set_xlabel("Node Index")
ax[1].set_ylabel("Density")
ax[1].legend()

ax[2].set_title("Inlet Velocity Profile [0]")
ax[2].set_ylabel("Velocity")
ax[2].set_xlabel("Node Index")
ax[2].legend()

ax[3].set_title("Inlet Velocity Profile [1]")
...
while t < TOTAL_TIME:
    ...
    u_avg_1 = u_avg[:, :, 1]   
    u_avg_0 = u_avg[:, :, 0]   
    #u_avg profiles at outlet and inlet
    u_avg_N_minus_1 = u_avg[:, :, Nx-1]
    u_avg_2 = u_avg[:, :, 2]    
    fi_x0, fi_xNplus1 = calcPeriodicBC(_roh_N, u_avg_N, roh_inlet, _roh_1, u_avg_1, roh_outlet, streamed_lattice)
    ...
    _ltc_pre = streamed_lattice.copy()
    streamed_lattice = streamLtc_nested(streamed_lattice)


    #5. Boundary Conditions
    #5.2 Bounce-Back Top and Bottom
    streamed_lattice = bounceBackTopBottom(streamed_lattice, _ltc_pre, _index_mapping_top, _index_mapping_bottom)  
    ...
    # Update plots
    roh_inlet_plot_0.set_xdata(range(num_nodes))
    roh_inlet_plot_0.set_ydata(roh_inlet)

    roh_inlet_plot_1.set_xdata(range(num_nodes))
    roh_inlet_plot_1.set_ydata(_roh_1)

    velocity_inlet_plot_0.set_xdata(range(num_nodes))
    velocity_inlet_plot_0.set_ydata(u_avg_0[0,:])

    velocity_inlet_plot_1.set_xdata(range(num_nodes))
    velocity_inlet_plot_1.set_ydata(u_avg_1[0,:])
