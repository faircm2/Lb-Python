...
if(VERBOSE): print("Shape of lattice:", lattice.shape)
...
def feq_i(i, rho, u_avg):
    ...
    if u_avg.ndim == 2:
        _dot = np.dot(c_i, u_avg.T)
    elif u_avg.ndim == 3:
        _dot = np.einsum('k,ijk->ij', c_i, u_avg)
    ...
    feq0_8 = weights[i] * rho * (_ones + 3.*np.dot(c_i, u_avg.T)/Cs**2 + (9./2)*_dot**2/Cs**4 - (3./2)*dot_product_u_avg/Cs**2)
...
def print_min_max(arr):
    # Get the overall minimum and maximum values for the input array
    arr_min = np.min(arr)
    arr_max = np.max(arr)
...
while t < TOTAL_TIME:
    ...
    if(VERBOSE): print("update extra node layers 0 and N+1 -> A): calculateXtraNodeLayers")    
    ...
    if(VERBOSE): print("update boundary conditions: _applyBounceBackTopBottom")    
    ...
    #update moments at N
    if(VERBOSE): print("update moments at N: updateMoments")  
