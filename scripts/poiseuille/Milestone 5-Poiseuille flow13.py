...
def bounceBackTopBottom00(_ltc, _ltc_pre, _index_mapping_top, _index_mapping_bottom):
...
def bounceBackTopBottom0(_ltc, _ltc_pre, _index_mapping_top, _index_mapping_bottom):
    ...
    # Post-stream bounce-back at top (reflect incoming from interior)
    _ltc[2, top_boundary, :] = _ltc_pre[4, top_boundary, :]  # [0, 1] <- [0, -1]
    _ltc[5, top_boundary, :] = _ltc_pre[7, top_boundary, :]  # [1, 1] <- [-1, -1]
    _ltc[6, top_boundary, :] = _ltc_pre[8, top_boundary, :]  # [-1, 1] <- [1, -1]

    # Post-stream bounce-back at bottom
    _ltc[4, bottom_boundary, :] = _ltc_pre[2, bottom_boundary, :]  # [0, -1] <- [0, 1]
    _ltc[7, bottom_boundary, :] = _ltc_pre[5, bottom_boundary, :]  # [-1, -1] <- [1, 1]
    _ltc[8, bottom_boundary, :] = _ltc_pre[6, bottom_boundary, :]  # [1, -1] <- [-1, 1]
...
im = ax[4].imshow(roh.T, interpolation='nearest', origin="upper")
...
def updatePlots(iteration, __pdf, _u_ckl):
    ...
    _roh_at_points_top = __roh[columns_to_select, Yn]
    _roh_at_points_mid = __roh[columns_to_select, (Yn+2)//2]
    _roh_at_points_bottom = __roh[columns_to_select, 1]
    ...
    if len(_roh_at_points_top) > 0 and iteration % n == 0:
        ...
        ax[2].plot(velocity_y_values, u_ckl[0, 1, 1:Yn+1], label="Numerical velocity profile - inlet", color="purple")
        ...
        ax[3].plot(velocity_y_values, u_ckl[0, Xn, 1:Yn+1], label="Numerical velocity profile - outlet", color="orange")
        ...
        im = ax[4].imshow(__roh.T, interpolation='nearest', origin = "upper")
        ax[4].set_aspect('auto')
