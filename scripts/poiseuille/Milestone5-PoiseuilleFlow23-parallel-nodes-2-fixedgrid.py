...
NODES = int(os.getenv("NODES", 2))  # Default to 2 if not set
_L = int(os.getenv("L", 1))       # Default to 1 if not set
Xn = int(os.getenv("Xn", NODES * 96 * 50))       # Default to 50*96*2 if not set
...
if rank == 0:
    ...
    path = f"Milestone5-PoiseuilleFlow23-parallel-{Yn}x{Xn}-nodes{NODES}-L{_L}-plot.png"
    plt.savefig(path)
    plt.close()
