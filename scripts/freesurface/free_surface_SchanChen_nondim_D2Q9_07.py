...
class FlowConfig:
    ...
    # defaults
    xi: float = 5.0                # default xi
    epsilon_cutoff: float = 1e-4   # can override
    alpha: float = 0.0             # default inclination in degrees
...
USE_CASES = {
    "proof_step": {"PHI_DISTRIBUTION": "STEP", "fc": PROOF_STEP},
...
iterationsOfInterest = get_iterations_of_interest(TOTAL_ITERATIONS, no_slices=NO_DATA_DUMP_SLICES, exp_factor=4.0)
density_slices = []
...
images_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "FreesurfaceImages")
os.makedirs(images_dir, exist_ok=True)
save_path = os.path.join(images_dir, f"{SCRIPT_FILENAME}_{ACTIVE_CASE}_Metrics_{TOTAL_ITERATIONS:0{FILENAME_PADDING_WIDTH}d}.png")
