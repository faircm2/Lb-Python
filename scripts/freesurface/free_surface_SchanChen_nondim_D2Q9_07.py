...
class FlowConfig:
    ...
    def TOTAL_ITERATIONS(self) -> int:
        ...
        return self.CORE_TOTAL_ITERATIONS * self.MULTIPLES
...
USE_CASES = {
    "proof_step": {"PHI_DISTRIBUTION": "STEP", "fc": PROOF_STEP5},
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    if CLIP_AND_SMOOTH_RHO_MU_PHI1:
        ...
        mu = np.clip(mu, fc.mu_G*0.5, fc.mu_L*2)
        _phi = np.clip(_phi, fc.phi_star_G*1.1, fc.phi_star_L*0.9)  # Confine to stable bulk
    ...
    if iteration == 0 or iteration==(fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0:
        if ADD_METRICS: 
            ...
            phi_left  = np.sum(_phi[x_left_idx , 1:Yn+1])   # sum over interior y-nodes
            phi_right = np.sum(_phi[x_right_idx, 1:Yn+1])
            PhiCollector.append((iteration, phi_left, phi_right))
...
if ADD_METRICS: 
    ...
    plotter.plot_bounds_ext(PhiCollector, "PhiCollector (Intg. _phi)", ax2[2, 3])
