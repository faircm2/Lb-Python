...
c = np.array([[0, 0],     # i=0
            ...
            [-1, -1],             # i=7
            [1, -1]])             # i=8
# D2Q9 lattice velocities c1 to c8 (exclude c0)
cx = np.array([ 1,  0, -1,  0,  1, -1, -1,  1])  # c_{i,x}
cy = np.array([ 0,  1,  0, -1,  1,  1, -1, -1])  # c_{i,y}
...
def c_first_derivative1(lamda, n_dx=1.0, n_dy=1.0):
    """
...
def c_first_derivative(lamda, n_dx=1.0, n_dy=1.0):
    """
    Inamuro Eq.(12) for D2Q9:
    ∂λ/∂x = (1/(10 Δx)) Σ_{i=1}^8 c_{i,x} λ(x + c_i Δt)
    Fast, vectorized, uses np.roll (valid for periodic or padded domains)
    """
    scale_x = 1.0 / (10.0 * n_dx)
    scale_y = 1.0 / (10.0 * n_dy)

    d_dx = np.zeros_like(lamda)
    d_dy = np.zeros_like(lamda)

    # Loop over 8 directions
    for i in range(8):
        # Shift field using lattice vector (c_x, c_y)
        shifted = np.roll(np.roll(lamda, cx[i], axis=0), cy[i], axis=1)
        d_dx += cx[i] * shifted
        d_dy += cy[i] * shifted
...
def c_second_derivative(lamda, n_dx=1.0, n_dy=1.0):
    ...
    (1/(3 Δx²)) * (sum_neighbors - 8 * lamda)
    Assumes lamda is scalar 2D field (nx, ny).
    """
    # Enforce 2D scalar input
    if len(lamda.shape) != 2:
        raise ValueError(f"Expected lamda shape (nx, ny) like (202,52), got {lamda.shape}")

    # CORRECT SCALING — INAMURO EQ.(13)
    scale_x = 1.0 / (3.0 * n_dx**2)
    scale_y = 1.0 / (3.0 * n_dy**2)

    # Roll axes for 2D: (0=x, 1=y)
    roll_axes = (0, 1)

    # Extract neighbor directions (k=1 to 8)
    neigh_c = c[1:9]  # Shape (8, 2)

    # Create shifted versions for all 8 directions
    shifted_neighbors = [
    ...
    # Vectorized sum over neighbors
    sum_neighbors = np.sum(neighbors_stack, axis=0)  # (nx, ny)
...
def initialize_nonlinear_phi(xn, yn, phi_star_g, phi_star_l, xi=5.0):
    """
...
def init_step_phi(xn, yn, phi_star_g, phi_star_l, xi=2.0, smooth_sigma=None):
    """
...
def periodic_boundary_conditions(_fi, _gi):
    _gi[:, 0, :] = _gi[:,Xn,:]
    _gi[:, Xn+1, :] = _gi[:,1,:]      
    _fi[:, 0, :] = _fi[:,Xn,:]
    _fi[:, Xn+1, :] = _fi[:,1,:]    
...
if PHI_NONLINEAR:
    # Example usage during initialization
    # Replace the original _phi initialization with a call to the method
    #_phi = initialize_nonlinear_phi(Xn+2, Yn+2, phi_star_G, phi_star_L, xi=5.0) 
...
while iteration < TOTAL_ITERATIONS:
    ...
    periodic_boundary_conditions(_fi, _gi)       
