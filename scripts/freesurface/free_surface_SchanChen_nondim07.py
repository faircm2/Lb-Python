...
CLIP_AND_SMOOTH_RHO_MU_PHI1 = True
...
c_x = c[:, 0][:, np.newaxis, np.newaxis] # (9,1,1)
c_y = c[:, 1][:, np.newaxis, np.newaxis] 
...
def gi(_gi, _gi_c, u_ckl, rho, mu, iteration):
    """
    Inamuro (2004) Eq. (3) – predicted velocity distribution gi.
    Uses c_first_derivative for *all* spatial derivatives.
    tau_g is the **global** relaxation parameter (same as tau_f but for momentum).
    """
    ...
    S_xx = 2.0 * du_x_dx
    S_yy = 2.0 * du_y_dy
    S_xy = du_y_dx + du_x_dy          # S_yx = S_xy

    # 3. Viscous stress σ = μ S
    sigma_xx = mu * S_xx
    sigma_yy = mu * S_yy
    sigma_xy = mu * S_xy
    sigma_yx = sigma_xy

    # 4. Divergence of σ via c_first_derivative
    d_sigma_xx_dx, _          = c_first_derivative(sigma_xx, n_dx, n_dy)
    _,        d_sigma_xy_dy   = c_first_derivative(sigma_xy, n_dx, n_dy)
    d_sigma_yx_dx, _          = c_first_derivative(sigma_yx, n_dx, n_dy)
    _,        d_sigma_yy_dy   = c_first_derivative(sigma_yy, n_dx, n_dy)

    div_sigma_x = d_sigma_xx_dx + d_sigma_xy_dy
    div_sigma_y = d_sigma_yx_dx + d_sigma_yy_dy

    # 5. Viscous forcing: 3 E_i c_i · (divσ / ρ) Δx
    div_x = div_sigma_x[np.newaxis, :, :]
    div_y = div_sigma_y[np.newaxis, :, :]
    rho_3d = rho[np.newaxis, :, :]

    c_dot = c_x * (div_x / rho_3d) + c_y * (div_y / rho_3d)
    viscous_term = 3.0 * E_exp * c_dot * n_dx

    # 6. BGK collision + forcing (tau_g is global)
    _gi_star = _gi - (1.0 / tau_g) * (_gi - _gi_c) + viscous_term

    # 7. Streaming
    for i in range(9):
        _gi[i] = np.roll(_gi_star[i], shift=(c[i, 0], c[i, 1]), axis=(0, 1))
...
def initialize_step_phi_2d(xn, yn, phi_star_g, phi_star_l, xi=2.0, smooth_sigma=None):
    """
...
if PHI_NONLINEAR:
    ...
    _phi = initialize_step_phi_2d(Xn+2,  Yn+2, phi_star_G, phi_star_L, xi=2.0, smooth_sigma=None)
