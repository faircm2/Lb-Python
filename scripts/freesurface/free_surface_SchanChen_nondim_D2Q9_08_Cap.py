...
def normalizePhi0(fc, _phi):  # Use your existing function that returns phi_n based on current min/max
    phi_min = np.min(_phi)
    phi_max = np.max(_phi)
    if phi_max == phi_min:
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    if False:
        # wall node indeces
        wall_indices = np.stack([np.arange(1, Xn+1), np.zeros(Xn, dtype=int)], axis=1)

        # first derivative of phi at the wall
        dphi_dx, dphi_dy = c_first_derivative(_phi)
        # normal gradient at wall (pointing into fluid)
        g_wall = dphi_dy[1:Xn+1, 1]

        # cubic boundary condition solution of quadratic eq(10) acc. to Zhang
        cos_th = np.cos(np.deg2rad(fc.vf_theta))
        #cubic boundary condition solution of quadratic eq(10) acc. to Zhang
        #phi_wall = _phi[1:Xn+1, 1] - np.sqrt(8.0 / fc.vf_W**2) * fc.vf_sigma * cos_th * g_wall
        #exact solution of quadratic eq(10) acc. Zhang

    #Zhang uses normalized phi, UInaumro uses absolute phi
    ...
    #Step 1: Normalize
    _phi_f_bottom = _phi[1:Xn+1, 1]  # Fluid node next to bottom wall (y=1)
    _phi_n, _phi_min, _phi_max = normalizePhi0(fc, _phi)
    _phi_f_n_bottom = _phi_n[1:Xn+1, 1]  # Normalized phi_f
    ...
    #_phi_w_n = np.clip(_phi_w_n, 0.0, 1.0)
    #_phi_w = denormalizePhi0(fc, _phi_w_n)
    _phi_w = denormalizePhi0(fc, _phi_w_n, _phi_min, _phi_max)
    #print("_phi_w min/max:", _phi_w.min(), _phi_w.max())
    #print("Expected:", fc.phi_star_G, fc.phi_star_L)
    if iteration % 100 == 0:
    ...
    #Bottom wall:
    _phi[1:Xn+1, 1] = _phi_w  # assign calculated _phi_w to bottom wall
    ...
    #top_wall_diff    = delta_phi[1:Xn+1, Yn+1]
    #print(
    #    f"Bottom wall dphi: min/max "
    #    f"{np.nanmin(bottom_wall_diff):.3e}, "
    #    f"{np.nanmax(bottom_wall_diff):.3e}"
    #)
    debug_log('ITER', 'Iter %d: Bottom wall dphi: min/max=%.3e, max=%.3e', iteration, np.nanmin(bottom_wall_diff), np.nanmax(bottom_wall_diff))
    #print(
    #    f"Top wall dphi: min/max "
    #    f"{np.nanmin(top_wall_diff):.3e}, "
    #    f"{np.nanmax(top_wall_diff):.3e}"
    ##)
    ...
    if iteration == 0 or iteration==(fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0:
        if ADD_METRICS:
            ...
            #DeltaPhiW.append((iteration, _bottom_wall_diff_min, _bottom_wall_diff_max, _top_wall_diff_min, _top_wall_diff_max))
            DeltaPhiW.append((iteration, _bottom_wall_diff_min, _bottom_wall_diff_max))
    # --------------------------------------------------------------


    #checmical potential acc. Inamuro eq() mu_c
