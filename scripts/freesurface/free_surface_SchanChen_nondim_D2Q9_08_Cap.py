...
def psi(fc, _phi, iteration=None):
    ...
    _psi = _phi * fc.T * np.log(_phi / (1 - fc.b * _phi)) - fc.a * _phi**2
    return _psi


def c_psi_phi(fc, _phi, iteration=None):
    """
    Inamuro eq(18): Compute partial derivative w.r.t. psi of bulk free-energy density.
    ∂ψ/∂ϕ = T ln(ϕ / (1 - bϕ)) + T / [ϕ (1 - bϕ)] - 2 a ϕ
    """
    _psi_phi = fc.T * np.log(_phi / (1-fc.b*_phi)) + fc.T / (_phi * (1-fc.b*_phi)) - 2*fc.a*_phi
    return _psi_phi
...
def compContactAngle(fc):
    ...
    _compContactAngle = -np.sqrt(2*beta/kappa) * cos_theta
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    fc_mu_c = c_psi_phi(fc, _phi) - fc.Kf * c_laplacian(_phi)
    surface_tension_force = fc_mu_c * c_first_derivative(_phi)
    if iteration == 0 or iteration==(fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0:
        if ADD_METRICS:
            _SurfaceTensionForce = np.max(np.abs(surface_tension_force))
            SurfaceTensionForceCollector.append((iteration, _SurfaceTensionForce))

    u_ckl_star = np.einsum('ia,ijk->ajk', c, _gi)  \
        + fc.ADD_BODY_FORCE * body_force \
        + fc.ADD_SURFACE_TENSION_FORCE * surface_tension_force 
...
if ADD_METRICS: 
    ...
    plotter.plot_bounds_ext(SurfaceTensionForceCollector, "Surface Tension Force", ax2[0, 3])
    ...
    series_labels = ["_bottom_wall_diff_min","_bottom_wall_diff_max"]
    #plotter.plot_bounds_ext(DivU_max, "DivU_max", ax2[2, 2])
    plotter.plot_bounds_ext(DeltaPhiW, "Delta _phi_w to _phi on boundary nodes", ax2[2, 2], series_labels)
