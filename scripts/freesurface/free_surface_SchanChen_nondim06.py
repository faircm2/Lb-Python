...
logger = logging.getLogger('lbm_debug')
...
post = np.floor((1 - np.exp(-exp * t)) * (TOTAL_ITERATION - 1 - fixed[-1])).astype(int) + fixed[-1]
...
while iteration < TOTAL_ITERATION:
    ...
    iteration += 1
    if iteration % 100 == 0:
        progress = (iteration / TOTAL_ITERATION) * 100.0
        debug_log('ITER', 'Simulation Execution -> TOTAL_ITERATION: %d; iteration: %d; %.1f %%', 
          TOTAL_ITERATION, iteration, progress)
