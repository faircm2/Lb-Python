...
def fi_c(fc, u, F, _phi):
    ...
    PhiTerms.append((iteration, _term3, _term4, _term6))
...
def bounce_back_walls_conservative(fi, gi, nx, ny):
    """
...
PARAMETER_STUB = "__tau_f_" + str(fc.tau_f) + "__Kf_" + str(fc.Kf)
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    _fi, _gi = bounce_back_walls_conservative(_fi, _gi, Xn, Yn)
...
if ADD_METRICS: 
    ...
    series_labels = ["-Kf*phi*d²phi/dx_a²", "-Kf/6*(dphi/dx_a)²", "E[i]*Kf*Gab(phi)*c_a[i]*c_b[i]"]
