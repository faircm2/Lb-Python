...
def bounceBackTopBottom_conservation(iteration, __fi, __gi, nx, ny):
    ...
    __fi[5, 1:nx+1, 1] = __fi[7, 1:nx+1, 0]   # NE ← SW + shift right
    __gi[5, 1:nx+1, 1] = __gi[7, 1:nx+1, 0]
    
    __fi[6, 1:nx+1, 1] = __fi[8, 1:nx+1, 0]   # NW ← SE + shift left    
    __gi[6, 1:nx+1, 1] = __gi[8, 1:nx+1, 0]


    # upper wall (y=ny) - no-slip halfway bounce-back
    __fi[4, 1 : nx + 1, ny] = __fi[2, 1 : nx + 1, ny + 1]
    __gi[4, 1 : nx + 1, ny] = __gi[2, 1 : nx + 1, ny + 1]

    __fi[7, 1 : nx + 1, ny] = __fi[5, 1 : nx + 1, ny + 1]
    __gi[7, 1 : nx + 1, ny] = __gi[5, 1 : nx + 1, ny + 1]

    __fi[8, 1 : nx + 1, ny] = __fi[6, 1 : nx + 1, ny + 1]
    __gi[8, 1 : nx + 1, ny] = __gi[6, 1 : nx + 1, ny + 1]
...
def bounceBackLeftRight_conservation(iteration, __fi, __gi, nx, ny):
    # Left wall (x=1) - no-slip halfway bounce-back
    __fi[1, 1, 1:ny+1] = __fi[3, 0, 1:ny+1]      # E ← W
    __gi[1, 1, 1:ny+1] = __gi[3, 0, 1:ny+1]

    __fi[5, 1, 1:ny+1] = __fi[7, 0, 1:ny+1]      # NE ← SW
    __gi[5, 1, 1:ny+1] = __gi[7, 0, 1:ny+1]

    __fi[8, 1, 1:ny+1] = __fi[6, 0, 1:ny+1]      # SE ← NW
    __gi[8, 1, 1:ny+1] = __gi[6, 0, 1:ny+1]
    ...
    __fi[7, nx, 1:ny+1] = __fi[5, nx+1, 1:ny+1]  # SW ← NE
    __gi[7, nx, 1:ny+1] = __gi[5, nx+1, 1:ny+1]

    __fi[6, nx, 1:ny+1] = __fi[8, nx+1, 1:ny+1]  # NW ← SE
    __gi[6, nx, 1:ny+1] = __gi[8, nx+1, 1:ny+1]
...
def surfaceTensionForces(iteration, fc, _phi, rho):
    ...
    #__phi[0, Yn+1]   = 0.5 * (_phi_w_top[0]   + _phi_w_left[-1])     # top-left
    #__phi[Xn+1, Yn+1]  = 0.5 * (_phi_w_top[-1]  + _phi_w_right[-1])    # top-right                
    #__phi[0, 0]   = 0.5 * (_phi_w_bottom[0]   + _phi_w_left[0])     # bottom-left
    #__phi[Xn+1, 0]  = 0.5 * (_phi_w_bottom[-1]  + _phi_w_right[0])    # bottom-right
...
zhang_surface_tension_force = np.zeros((2, 52, 52))
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    _fi, _gi = bounceBackTopBottom_conservation(iteration, _fi_star, _gi_star, Xn, Yn)
    

    #4.1b. top/bottom conservative bounceback
    #apply_periodic_boundary_conditions(_fi, _gi)    
    _fi, _gi = bounceBackLeftRight_conservation(iteration, _fi_star, _gi_star, Xn, Yn)
    ...
    if fc.ADD_SURFACE_TENSION_FORCE == 1:
        __phi, zhang_surface_tension_force = surfaceTensionForces(iteration, fc, _phi, rho)                                    
        _phi = __phi
