...
#in this simulation we model the cross-section of a chanel and see whether the capillary force component actually works
#square channel WITHOUT periodic boundary conditions, just bounceback on all 4 boundaries
#CAPILLARY -> model square cross section
...
class FlowConfig:
    ...
    alpha: float = 0.0                  # inclination (degrees)
...
def psi(fc, _phi, iteration=None):
    """
    Inamuro eq(11): Bulk free-energy density ψ(ϕ).
    ψ = ϕ T ln(ϕ / (1 - bϕ)) - a ϕ²
    """
...
def p0(fc, __phi):
    _p0 = ((__phi * fc.T) / (1 - fc.b * __phi)) - fc.a * __phi**2
...
def bounceBackLeftRight_conservation(fi, gi, nx, ny):
    """
    Fully conservative + physically correct bounce-back for left/right walls
    Works with D2Q9, ghost layers already filled by mirror/extrapolation
    Arrays are shape (9, nx+2, ny+2)
    """

    # === Left wall (x = 1) ===
    # Straight directions
    fi[1, 1, 1:ny+1] = fi[3, 0, 1:ny+1]    # E ← W (from ghost)
    gi[1, 1, 1:ny+1] = gi[3, 0, 1:ny+1]

    # Diagonals — pure reflection, NO ROLL
    fi[5, 1, 1:ny+1] = fi[7, 0, 1:ny+1]    # NE ← SW
    fi[8, 1, 1:ny+1] = fi[6, 0, 1:ny+1]    # SE ← NW

    gi[5, 1, 1:ny+1] = gi[7, 0, 1:ny+1]
    gi[8, 1, 1:ny+1] = gi[6, 0, 1:ny+1]

    # === Right wall (x = nx) ===
    fi[3, nx, 1:ny+1] = fi[1, nx+1, 1:ny+1]
    fi[7, nx, 1:ny+1] = fi[5, nx+1, 1:ny+1]
    fi[6, nx, 1:ny+1] = fi[8, nx+1, 1:ny+1]

    gi[3, nx, 1:ny+1] = gi[1, nx+1, 1:ny+1]
    gi[7, nx, 1:ny+1] = gi[5, nx+1, 1:ny+1]
    gi[6, nx, 1:ny+1] = gi[8, nx+1, 1:ny+1]
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    #CAPILLARY FLOW -> no periodic flow, instead bounceback lef/right
    ...
    #Krüger et al, p. 241 eq. (6.29) & Table 6.1
    #1. Shan-Chen - A=tau*n_dt
    A = n_dt*n_dt

    # 1. body mass force
    body_force = A*force_(fc.F_lattice, rho)*n_dt
    ...
    # ------- Capillary effect -------------------------------------------------------
    # Calculate _phi_w acc. Zhang
    # Zhang uses normalized phi, Inaumro uses absolute phi
    #   -> normalize phi from Inamuro simulation, 
    #   -> calculate wall value of phi, phi_W, using Zhang approach
    #   -> denormalize _phi_w and overwrite the values of phi at the wall

    phi_before = _phi.copy()

    #Step 1: Normalize
    # maps Inamuro phi_f to [0,1] normalized space used by Zhang
    _phi_n, _phi_min, _phi_max = normalizePhi0(fc, _phi)
    _phi_f_n_bottom = _phi_n[1:Xn+1, 1]  # Fluid node next to bottom wall (y=1)


    #Step 2: Apply Zhang boundary condition
    thetaCap = compContactAngle(fc)
    # solve Zhang Eq. (10) in normalized phi
    _phi_w_n = calc_phi_w(_phi_f_n_bottom , thetaCap, n_dx)


    #Step 3: Denormalize back to Inamuro’s phi
    _phi_w = denormalizePhi0(fc, _phi_w_n, _phi_min, _phi_max)
    if iteration % 100 == 0:
        debug_log('ITER', 'Iter %d: Expected phi:=%.3e, max=%.3e', iteration, fc.phi_star_G, fc.phi_star_L)
        debug_log('ITER', 'Iter %d: _phi_w min/max=%.3e, max=%.3e', iteration, _phi_min, _phi_max)

    # Assign calculated _phi_w to bottom wall
    _phi[1:Xn+1, 1] = _phi_w  

    # Assign calculated _phi_w to top wall -> not relevant due to air
    #_phi[1:Xn+1, Yn+1] = _phi_w[1:Xn+1, Yn+1]  # assign calculated _phi_w to top wall

    # Compare _phi to _phi_w
    phi_after = _phi.copy()
    delta_phi = phi_after - phi_before
    bottom_wall_diff = delta_phi[1:Xn+1, 1]
    debug_log('ITER', 'Iter %d: Bottom wall dphi: min/max=%.3e, max=%.3e', iteration, np.nanmin(bottom_wall_diff), np.nanmax(bottom_wall_diff))
    #debug_log('ITER', 'Iter %d: Top wall dphi: min/max=%.3e, max=%.3e', iteration, np.nanmin(top_wall_diff), np.nanmax(top_wall_diff))
    if iteration == 0 or iteration==(fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0:
        if ADD_METRICS:
            _bottom_wall_diff_min = np.nanmin(bottom_wall_diff)
            _bottom_wall_diff_max = np.nanmax(bottom_wall_diff)
            #_top_wall_diff_min = np.nanmin(top_wall_diff)
            #_top_wall_diff_max = np.nanmax(top_wall_diff)    
            DeltaPhiW.append((iteration, _bottom_wall_diff_min, _bottom_wall_diff_max))
    # --------------------------------------------------------------
    ...
    zhang_surface_tension_force = st_mu_phi(fc, _phi) * c_first_derivative(_phi)
    if iteration == 0 or iteration==(fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0:
        if ADD_METRICS:
            ...
            SurfaceTensionForceCollector.append((iteration, _InamuroSurfaceTensionForce, _ZhangSurfaceTensionForce))
    ...
    u_ckl_star = np.einsum('ia,ijk->ajk', c, _gi)  \
        ...
        #+ fc.ADD_SURFACE_TENSION_FORCE * inamuro_surface_tension_force 
...
if ADD_METRICS: 
    ...
    series_labels = ["Inamuro Surface-Tension Force", "Zhang Surface-Tension Force"]
    plotter.plot_bounds_ext(SurfaceTensionForceCollector, "Surface Tension Force", ax2[0, 3], series_labels)
