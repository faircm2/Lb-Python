...
class FlowConfig:
    ...
    # Max no. iterations to achieve convergence in pressure correction loop, see Inamuro eq(22)
    max_ph_iters: int = 10000
    ...
    rho_G: float = 1.0
    rho_L: float = 50.0
    phi_star_G: float = 1.5e-2
    phi_star_L: float = 9.2e-2
    ...
    def mu_G(self) -> float:
        ...
        return 1.6e-4 * self.n_dx

    @property
    def mu_L(self) -> float:
        """Liquid-phase dynamic viscosity (lattice units)."""
        return 8.0e-3 * self.n_dx
...
def psi(fc, _phi, iteration=None):
    """
    ...
    validate_field(_phi, '_phi', iter=iteration, allow_range=(0, 1/fc.b))
    term1 = _phi * fc.T * np.log(_phi / (1.0 - fc.b * _phi)) 
    term2 = - fc.a * _phi**2
    _psi = term1 + term2
    return _psi


#First derivative of Inamuro eq(11) -> for calculating chemcial potential acc. to eq(18)
def c_psi_phi(fc, _phi, iteration=None):
    """
    Partial derivative ∂ψ/∂ϕ from Inamuro eq(11).
    ∂ψ/∂ϕ = T ln(ϕ / (1 - bϕ)) + T ϕ / (1 - bϕ) + T (1 - ϕ b) / (1 - bϕ) wait no:
    Correct derivation: T ln(ϕ / (1-bϕ)) + T ϕ b / (1-bϕ) + T / (1-bϕ) - 2 a ϕ
    But simplifies exactly to:
    ∂ψ/∂ϕ = T ln(ϕ / (1 - bϕ)) + T / (1 - bϕ) - 2 a ϕ
    (the extra ϕ b term cancels with part of the log derivative)
    """
    log_term = np.log(_phi / (1.0 - fc.b * _phi))
    denom_term = fc.T / (1.0 - fc.b * _phi)

    _psi_phi = fc.T * log_term + denom_term - 2.0 * fc.a * _phi
    return _psi_phi
...
def mu_c(fc, __phi):
    ...
    """
    psi_deriv = c_psi_phi(fc, _phi)
    lap_phi = c_laplacian(_phi)  # n_dx/n_dy handled inside c_laplacian as needed
...
def p0(fc, _phi):
    _p0 = ((_phi * fc.T) / (1 - fc.b * _phi)) - fc.a * _phi**2
...
def gradient_p(p):
    ...
    dp_dx, dp_dy = c_first_derivative0(p, n_dx, n_dy)
...
def c_first_derivative0(lamda, n_dx=1.0, n_dy=1.0):
...
def c_second_derivative(lamda, n_dx=1.0, n_dy=1.0):
    ...
    scale_x = 1.0/(3.0 * n_dx**2) #(8.0/14.0) / (5.0 * n_dx)
    scale_y = 1.0/(3.0 * n_dy**2) #(8.0/14.0) / (5.0 * n_dy)
...
def c_laplacian(lamda, n_dx=1.0, n_dy=1.0):
    ...
    d2lamda_dx2_old, d2lamda_dy2_old = c_second_derivative(lamda, n_dx, n_dy)
    laplacian_old = d2lamda_dx2_old + d2lamda_dy2_old

    if iteration == 0 or iteration==(fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0:
        # Check if there is any difference
        diff_exists = not np.allclose(laplacian, laplacian_old)
        # Maximum absolute difference
        max_diff = np.max(np.abs(laplacian - laplacian_old))

        print(f"c_laplacian - max_diff: {max_diff}")
...
def Gab(_phi):
    dphi_x, dphi_y = c_first_derivative0(_phi)
...
def density_and_viscosity(fc, _phi):
    ...
    delta_rho = fc.rho_L - fc.rho_G
    phi_dash_star = (fc.phi_star_L + fc.phi_star_G) / 2
    phi_delta_star = fc.phi_star_L - fc.phi_star_G   

    #if _phi < phi_star_G:
    #    _rho = rho_G
    mask1 = _phi < fc.phi_star_G
    _rho[mask1] = fc.rho_G
    #elif _phi >= phi_star_G and _phi <= phi_star_L:
    #    _rho = rho_center
    mask2 = (_phi >= fc.phi_star_G) & (_phi <= fc.phi_star_L)
    rho_center = (delta_rho/2) * (np.sin(np.pi * ((_phi[mask2] - phi_dash_star)/phi_delta_star)) + 1) + fc.rho_G
    _rho[mask2] = rho_center
    #elif _phi > phi_star_L:
    #    _rho = rho_L
    mask3 = _phi > fc.phi_star_L
    _rho[mask3] = fc.rho_L

    _mu = ((_rho - fc.rho_G) / (fc.rho_L - fc.rho_G)) * (fc.mu_L - fc.mu_G) + fc.mu_G
...
def gi(fc, _gi, _gi_c, u_ckl, rho, mu, iteration):
    """
    ...
    use_central_difference_scheme = False

    # Velocity gradients on full grid (nx, ny = 202,52)
    grads = [c_first_derivative0(u_ckl[i], n_dx, n_dy) for i in range(2)]
    ...
    if use_central_difference_scheme:
        ...
        d_sigma_xx_dx = c_first_derivative0(sigma_xx, n_dx, n_dy)[0]
        d_sigma_xy_dy = c_first_derivative0(sigma_xy, n_dx, n_dy)[1]
        div_sigma_x = d_sigma_xx_dx + d_sigma_xy_dy

        d_sigma_yx_dx = c_first_derivative0(sigma_yx, n_dx, n_dy)[0]
        d_sigma_yy_dy = c_first_derivative0(sigma_yy, n_dx, n_dy)[1]
        div_sigma_y = d_sigma_yx_dx + d_sigma_yy_dy        
    ...
    _gi_star = _gi_old - (1.0 / fc.tau_g) * (_gi_old - _gi_c) + viscous_term
...
def gi_c(fc, u, rho, iteration):
    ...
    grad_rho_x, grad_rho_y = c_first_derivative0(rho, n_dx, n_dy)  # (nx_pad, ny_pad)
    u_dot_u = np.sum(u**2, axis=0)  # (nx_pad, ny_pad)
    
    # Velocity gradients on full
    du_a_dx, du_a_dy = c_first_derivative0(u[0], n_dx, n_dy)
    du_b_dx, du_b_dy = c_first_derivative0(u[1], n_dx, n_dy)
...
def fi(fc, _fi, _fi_c):
    ...
    #_fi_star = _fi_old + omega_f * (_fi_c - _fi_old)  # Broadcast all
...
def fi_c(fc, u, F, _phi):
    ...
    dphi_dxa, dphi_dya = c_first_derivative0(_phi, n_dx, n_dy)  # (nx_pad, ny_pad)
    
    laplacian_phi = c_laplacian(_phi, n_dx, n_dy)  # (nx_pad, ny_pad)
    
    G = Gab(_phi)  # (nx_pad, ny_pad, 2, 2)
    
    p0_val = p0(fc, _phi)  # (nx_pad, ny_pad)
    ...
    term3 = F_exp * fc.Kf * _phi * laplacian_phi
    
    # term4: F[i] * Kf/6 * (dphi_dxa**2 + dphi_dya**2)
    term4 = F_exp * (fc.Kf / 6.0) * (dphi_dxa**2 + dphi_dya**2)
    ...
    term6 = E_exp * fc.Kf * _Gab
    
    # Assemble _fi_c
    _fi_c = term1 + term2 - term3 - term4 + term5 + term6  # (9,nx_pad,ny_pad)

    _term3 = - np.max(np.abs(-term3))
    _term4 = - np.max(np.abs(-term4))
    _term5 = np.max(np.abs(term5))
    _term6 = np.max(np.abs(term6))
    PhiTerms.append((iteration, _term3, _term4, _term5, _term6))

    # mass conservation correction for phi_ growth
    if fc.ENFORCE_MASS_CONSERVATION:
        sum_fi_c = np.sum(_fi_c, axis=0)  # (Xn+2, Yn+2)
        correction = E_exp * (_phi - sum_fi_c)  # Distribute difference using weights E_i
        _fi_c += correction    
   
    return _fi_c


def bounceBackTopBottom_conservation_halfway_bounce_back(fi, gi, nx, ny):
    """
    No-slip bounce-back on BOTTOM wall only.
    TOP wall: open (zero-gradient extrapolation into ghost layer y=ny+1)
    
    Arrays shape: (9, nx+2, ny+2)
    Fluid domain: x=1…nx, y=1…ny
    """
    # ────────────────────────────────────────────────
    # Bottom wall (y=1) - no-slip bounce-back
    # using ghost layer y=0
    # ────────────────────────────────────────────────
    # Straight directions
    fi[2, 1:nx+1, 1] = fi[4, 1:nx+1, 0]    
    gi[2, 1:nx+1, 1] = gi[4, 1:nx+1, 0]

    # Diagonals – shift
    fi[5, 1:nx+1, 1] = np.roll(fi[7, 1:nx+1, 0],  1)  
    gi[5, 1:nx+1, 1] = np.roll(gi[7, 1:nx+1, 0],  1)

    fi[6, 1:nx+1, 1] = np.roll(fi[8, 1:nx+1, 0], -1)   
    gi[6, 1:nx+1, 1] = np.roll(gi[8, 1:nx+1, 0], -1)

    # ────────────────────────────────────────────────
    # Top wall (y=ny) → Open boundary (zero normal gradient)
    # Copy last fluid row → ghost row
    # ────────────────────────────────────────────────
    fi[1 : nx + 1, ny, 4] = fi[1 : nx + 1, ny + 1, 2]
    fi[1 : nx + 1, ny, 7] = np.roll(fi[1 : nx + 1, ny + 1, 5], -1)
    fi[1 : nx + 1, ny, 8] = np.roll(fi[1 : nx + 1, ny + 1, 6], 1)

    gi[1 : nx + 1, ny, 4] = gi[1 : nx + 1, ny + 1, 2]
    gi[1 : nx + 1, ny, 7] = np.roll(gi[1 : nx + 1, ny + 1, 5], -1)
    gi[1 : nx + 1, ny, 8] = np.roll(gi[1 : nx + 1, ny + 1, 6], 1)    

    return fi, gi


def bounceBackLeftRight_conservation_halfway_bounce_back(fi, gi, nx, ny):
    """
    No-slip bounce-back on LEFT and RIGHT walls using ghost layers (x=0 and x=nx+1).
    Uses classic bounce-back + diagonal roll (shift) for consistency with horizontal walls.
    
    Arrays shape: (9, nx+2, ny+2)
    Fluid domain: x=1…nx, y=1…ny
    """
    # ────────────────────────────────────────────────
    # Left wall (x=1) - no-slip using ghost x=0
    # ────────────────────────────────────────────────
    # Straight directions
    fi[1, 1, 1:ny+1] = fi[3, 0, 1:ny+1]    
    gi[1, 1, 1:ny+1] = gi[3, 0, 1:ny+1]

    # Diagonals – shift
    # up (y+1 shift)
    fi[5, 1, 1:ny+1] = np.roll(fi[7, 0, 1:ny+1],  1, axis=0)
    gi[5, 1, 1:ny+1] = np.roll(gi[7, 0, 1:ny+1],  1, axis=0)
    # down (y-1 shift)
    fi[8, 1, 1:ny+1] = np.roll(fi[6, 0, 1:ny+1], -1, axis=0)
    gi[8, 1, 1:ny+1] = np.roll(gi[6, 0, 1:ny+1], -1, axis=0)

    # ────────────────────────────────────────────────
    # Right wall (x=nx) - no-slip using ghost x=nx+1
    # ────────────────────────────────────────────────
    # Straight directions
    fi[3, nx, 1:ny+1] = fi[1, nx+1, 1:ny+1] 
    gi[3, nx, 1:ny+1] = gi[1, nx+1, 1:ny+1]

    # Diagonals – shift
    # up (y+1 shift)
    fi[7, nx, 1:ny+1] = np.roll(fi[5, nx+1, 1:ny+1], -1, axis=0)  
    gi[7, nx, 1:ny+1] = np.roll(gi[5, nx+1, 1:ny+1], -1, axis=0)
    # down (y-1 shift)
    fi[6, nx, 1:ny+1] = np.roll(fi[8, nx+1, 1:ny+1],  1, axis=0)  
    gi[6, nx, 1:ny+1] = np.roll(gi[8, nx+1, 1:ny+1],  1, axis=0)

    return fi, gi


def bounceBackTopBottom_conservation(fi, gi, nx, ny):
    ...
    fi[2, 1:nx+1, 1] = fi[4, 1:nx+1, 0+ghost_layer_addition]   # N ← S
    gi[2, 1:nx+1, 1] = gi[4, 1:nx+1, 0+ghost_layer_addition]

    fi[5, 1:nx+1, 1] = fi[7, 1:nx+1, 0+ghost_layer_addition]   # NE ← SW + shift right
    gi[5, 1:nx+1, 1] = gi[7, 1:nx+1, 0+ghost_layer_addition]
    
    fi[6, 1:nx+1, 1] = fi[8, 1:nx+1, 0+ghost_layer_addition]   # NW ← SE + shift left    
    gi[6, 1:nx+1, 1] = gi[8, 1:nx+1, 0+ghost_layer_addition]

    # upper wall (y=ny) - no-slip halfway bounce-back
    fi[4, 1 : nx + 1, ny] = fi[2, 1 : nx + 1, ny + 1-ghost_layer_addition]
    gi[4, 1 : nx + 1, ny] = gi[2, 1 : nx + 1, ny + 1-ghost_layer_addition]

    fi[7, 1 : nx + 1, ny] = fi[5, 1 : nx + 1, ny + 1-ghost_layer_addition]
    gi[7, 1 : nx + 1, ny] = gi[5, 1 : nx + 1, ny + 1-ghost_layer_addition]

    fi[8, 1 : nx + 1, ny] = fi[6, 1 : nx + 1, ny + 1-ghost_layer_addition]
    gi[8, 1 : nx + 1, ny] = gi[6, 1 : nx + 1, ny + 1-ghost_layer_addition]

    return fi, gi    


def bounceBackLeftRight_conservation(fi, gi, nx, ny):
    ghost_layer_addition = 0
    # Left wall (x=1) - no-slip halfway bounce-back
    fi[1, 1, 1:ny+1] = fi[3, 0+ghost_layer_addition, 1:ny+1]      # E ← W
    gi[1, 1, 1:ny+1] = gi[3, 0+ghost_layer_addition, 1:ny+1]

    fi[5, 1, 1:ny+1] = fi[7, 0+ghost_layer_addition, 1:ny+1]      # NE ← SW
    gi[5, 1, 1:ny+1] = gi[7, 0+ghost_layer_addition, 1:ny+1]

    fi[8, 1, 1:ny+1] = fi[6, 0+ghost_layer_addition, 1:ny+1]      # SE ← NW
    gi[8, 1, 1:ny+1] = gi[6, 0+ghost_layer_addition, 1:ny+1]

    # Right wall (x=nx) - no-slip halfway bounce-back
    fi[3, nx, 1:ny+1] = fi[1, nx+1-ghost_layer_addition, 1:ny+1]  # W ← E
    gi[3, nx, 1:ny+1] = gi[1, nx+1-ghost_layer_addition, 1:ny+1]

    fi[7, nx, 1:ny+1] = fi[5, nx+1-ghost_layer_addition, 1:ny+1]  # SW ← NE
    gi[7, nx, 1:ny+1] = gi[5, nx+1-ghost_layer_addition, 1:ny+1]

    fi[6, nx, 1:ny+1] = fi[8, nx+1-ghost_layer_addition, 1:ny+1]  # NW ← SE
    gi[6, nx, 1:ny+1] = gi[8, nx+1-ghost_layer_addition, 1:ny+1]
...
def init_horizontal_phi(xn, yn, phi_star_g, phi_star_l, height=None, xi=5.0):
    """
...
def calc_phi_w(phi_f, thetaCap, n_dx):
    """
    Compute wet-node wall value __phi_w using Zhang's Eq. (10)
    ...
    """
    discriminant = (thetaCap + 1.0/n_dx)**2 - 4.0 * thetaCap * phi_f / n_dx
    __phi_w = ((thetaCap + 1.0/n_dx) - np.sqrt(discriminant)) / (2.0 * thetaCap)
    
    return __phi_w


def calc_phi_s(_phi_p, theta, thetaCap, n_dx):
    """
...
def calc_phi_w_2ndorder_wet(phi_f1, phi_f2, thetaCap, n_dx, rootOption="-"):
    """
    Compute wet-node wall value __phi_w using Zhang's Eq. (10)
    ...
    __phi_w : float or array
        Value(s) at wall node
    """
    discriminant = (thetaCap + 3.0/(2.0*n_dx))**2 - 4.0 * thetaCap * (4.0*phi_f1-phi_f2) / (2.0*n_dx)
    if rootOption == "-":
        __phi_w = ((thetaCap + 3.0/(2.0*n_dx)) - np.sqrt(discriminant)) / (2.0 * thetaCap)
    else:
        __phi_w = ((thetaCap + 3.0/(2.0*n_dx)) + np.sqrt(discriminant)) / (2.0 * thetaCap)
...
def calc_phi_w_2ndorder_halfway_bounce_back(phi_f1, phi_f2, thetaCap, n_dx, rootOption="-"):
    """
    Compute wet-node wall value __phi_w using Zhang's Eq. (10)
    ...
    """
    a = 3.0*n_dx*thetaCap
    b = -(3.0 * n_dx * thetaCap + 8.0)
    c = 9 * phi_f1 - phi_f2
    discriminant = b**2 - 4.0*a*c
    if rootOption == "-":
        __phi_w = (-b - np.sqrt(discriminant)) / (2.0*a)
    else:
        __phi_w = (-b + np.sqrt(discriminant)) / (2.0*a)
...
def normalizePhi1(fc, phi):
    phi_min = np.min(_phi)
    phi_max = np.max(_phi)
    phi_n = (phi - fc.phi_star_G) / (fc.phi_star_L - fc.phi_star_G)
    return phi_n, phi_min, phi_max

def normalizePhi0(fc, _phi):  
    phi_min = np.min(_phi)
    phi_max = np.max(_phi)
    if phi_max == phi_min:
        return np.zeros_like(_phi)
    phi_n = (_phi - phi_min) / (phi_max - phi_min)
    return phi_n, phi_min, phi_max  # Return min/max for denormalization

def denormalizePhi0(fc, phi_n, phi_min, phi_max):
    phi_dn = phi_min + phi_n * (phi_max - phi_min)
    return phi_dn

def denormalizePhi1(fc, phi_n, phi_min, phi_max):
    phi_dn = fc.phi_star_G + phi_n * (fc.phi_star_L - fc.phi_star_G)
    return phi_dn
...
def compute_m_vertical_wall(
    ...
    """m for vertical wall as 2-component array [mx, my]."""
    gx, gy = dphidx[i, j], dphidy[i, j]
    grad_norm = np.hypot(gx, gy)
    if grad_norm < eps:
        return np.zeros(2, dtype=np.float64)

    nx, ny = gx / grad_norm, gy / grad_norm

    if wall.lower() in ("left", "west"):
        mx_star = -ny**2
        my_star = nx * ny
    elif wall.lower() in ("right", "east"):
        mx_star = ny**2
        my_star = -nx * ny
    else:
        raise ValueError("wall must be 'left'/'west' or 'right'/'east'")

    m_norm = np.hypot(mx_star, my_star)
    if m_norm < eps:
        return np.zeros(2, dtype=np.float64)

    return sign * np.array([mx_star / m_norm, my_star / m_norm], dtype=np.float64)


def compute_m_horizontal_wall(
    dphidx: np.ndarray,
    dphidy: np.ndarray,
    i: int,
    j: int,
    ...
    """m for horizontal wall as 2-component array [mx, my]."""
    gx, gy = dphidx[i, j], dphidy[i, j]
    grad_norm = np.hypot(gx, gy)
    if grad_norm < eps:
        return np.zeros(2, dtype=np.float64)
    ...
    if wall.lower() in ("bottom", "south"):
        mx_star = nx * ny
        my_star = -nx**2
    elif wall.lower() in ("top", "north"):
        mx_star = -nx * ny
        my_star = nx**2
    else:
        raise ValueError("wall must be 'bottom'/'south' or 'top'/'north'")
...
def n0(_phi):
    # 1) Compute gradient of phi: ∇phi
    dphi_dy, dphi_dx = c_first_derivative0(_phi, n_dy, n_dx)  # note: (y, x) order

    # 2) Compute magnitude |∇phi|
    grad_mag = np.sqrt(dphi_dx**2 + dphi_dy**2)

    # 3) Normalize: n = ∇phi / |∇phi|
    n_x = dphi_dx / (grad_mag + 1e-12)  # small term avoids divide-by-zero
    n_y = dphi_dy / (grad_mag + 1e-12)

    # Stack into a single vector field (optional)
    _n = np.stack([n_x, n_y], axis=-1)
...
def n1(_phi):
    # 1) Compute gradient of phi: ∇phi
    dphi_dy, dphi_dx = c_first_derivative0(_phi)  # note: (y, x) order
...
def surfaceTensionForces(iteration, fc, _phi, rho):
    ...
    #normalization test
    _phi_n, _phi_min, _phi_max = normalizePhi0(fc, _phi)
    _phi_post = denormalizePhi0(fc, _phi_n, _phi_min, _phi_max)

    diff = _phi - _phi_post
    max_diff = np.max(np.abs(diff))

    tol = 1e-12   # or 1e-9 if you prefer

    if max_diff > tol:
        print("# ───── normalization test ─────────────────────────────────────")
        print("WARNING: normalization round-trip error")
        print(f"max |phi - phi_post| = {max_diff:.3e}")
        print(f"tolerance            = {tol:.1e}")

        idx = np.unravel_index(np.argmax(np.abs(diff)), diff.shape)
        print("worst index:", idx)
        print("_phi      =", _phi[idx])
        print("_phi_post =", _phi_post[idx])
        print("# ──────────────────────────────────────────────────────────────")        
    #else:
    #    print(f"OK: normalization within tolerance ({tol:.1e})")
    ...
    # === Top wall (y=1) ===
    _phi_f_n_top = _phi_n[:, Yn]                    # 1. fluid node inside
    _phi_w_n_top = calc_phi_s(_phi_f_n_top, fc.vf_theta, thetaCap, n_dx)   

    # === Bottom wall (y=1) ===
    _phi_f_n_bottom = _phi_n[:, 1]                    # 1. fluid node inside
    _phi_w_n_bottom = calc_phi_s(_phi_f_n_bottom, fc.vf_theta, thetaCap, n_dx)           

    # === Left wall (x=1) ===
    _phi_f_n_left = _phi_n[1, :]                    # 1. fluid node inside
    _phi_w_n_left = calc_phi_s(_phi_f_n_left, fc.vf_theta, thetaCap, n_dx)                   

    # === Right wall (x=Xn) ===
    _phi_f_n_right = _phi_n[Xn, :]                    # 1. fluid node inside
    _phi_w_n_right = calc_phi_s(_phi_f_n_right, fc.vf_theta, thetaCap, n_dx)                           


    # Step 2: Denormalize back to Inamuro scale
    _phi_w_top = denormalizePhi0(fc, _phi_w_n_top, _phi_min, _phi_max)
    _phi_w_bottom = denormalizePhi0(fc, _phi_w_n_bottom, _phi_min, _phi_max)
    _phi_w_left   = denormalizePhi0(fc, _phi_w_n_left,   _phi_min, _phi_max)
    _phi_w_right  = denormalizePhi0(fc, _phi_w_n_right,  _phi_min, _phi_max)


    # Step 3: Assign to solid wall nodes only
    # Top wall
    _phi[:, Yn+1] = _phi_w_top  # current x=Xn+1  
    _phi_n[:, Yn+1] = _phi_w_n_top  # current x=Xn+1  

    # Bottom wall
    _phi[:, 0]  = _phi_w_bottom   # bottom
    _phi_n[:, 0]  = _phi_w_n_bottom   # bottom

    # Left wall
    _phi[0, :] = _phi_w_left   # current x=0
    _phi_n[0, :] = _phi_w_n_left   # current x=0

    # Right wall
    _phi[Xn+1, :] = _phi_w_right  # current x=Xn+1  
    _phi_n[Xn+1, :] = _phi_w_n_right  # current x=Xn+1  


    _zhang_fc_phi_n = zhang_fc(fc, _phi_n)
    _zhang_fc_phi = zhang_fc(fc, _phi)
    #if ADD_METRICS and (iteration == 0 or iteration == (fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0):
    if ADD_METRICS and iteration in iterationsOfInterest:
        fcBounds_left.append((iteration,
                        _zhang_fc_phi_n[0,1,:].copy(), # norm. left-x
                        _zhang_fc_phi_n[1,1,:].copy(), # norm. left-y
                        _zhang_fc_phi[0,1,:].copy(), # left-x  
                        _zhang_fc_phi[1,1,:].copy())) # left-y 
        fcBounds_right.append((iteration,
                        _zhang_fc_phi_n[0,Xn,:].copy(), # norm. right-x
                        _zhang_fc_phi_n[1,Xn,:].copy(), # norm. right-y
                        _zhang_fc_phi[0,Xn,:].copy(), # right-x  
                        _zhang_fc_phi[1,Xn,:].copy())) # right-y   

        iteration, norm_left_x, norm_left_y, left_x, left_y = fcBounds_left[-1]  # or .pop() if you want to remove
        plotter.phi_boundary_forces_vertical(
                    norm_left_x, norm_left_y, left_x, left_y,
                    label_left_x="norm. Left wall Fx",
                    label_left_y="norm. Left wall Fy",
                    label_right_x="Left wall Fx",
                    label_right_y="Left wall Fy",
                    yc=yc,
                    iteration=iteration,
    ...
    # === Corner averaging for solid corners only (bottom-left and bottom-right) ===
    #_phi[0, Yn+1]   = 0.5 * (_phi_w_top[0]   + _phi_w_left[-1])     # top-left
    #_phi[Xn+1, Yn+1]  = 0.5 * (_phi_w_top[-1]  + _phi_w_right[-1])    # top-right                
    #_phi[0, 0]   = 0.5 * (_phi_w_bottom[0]   + _phi_w_left[0])     # bottom-left
    #_phi[Xn+1, 0]  = 0.5 * (_phi_w_bottom[-1]  + _phi_w_right[0])    # bottom-right
         

    # CRITICAL: Restore exact mass conservation after overwrite
    # Compute total ϕ in fluid domain AFTER wetting
    phi_total_after = np.sum(_phi[1:Xn+1, 1:Yn+1])
    ...
    if fc.ENFORCE_MASS_CONSERVATION and abs(phi_total_after - phi_old_total) > 1e-8:  # avoid div-by-zero or noise
        ...
        _phi[1:Xn+1, 1:Yn+1] *= scale_factor
    ...
    # surface_tension_force Zhang eq(5) using chemical potential acc. Inamuro eq(18) mu_c
    # scale with division by  / fc.CF analog """Body force vector scaled to lattice units."""   
    inamuro_surface_tension_force = mu_c(fc, _phi) * c_first_derivative0(_phi) / (2.0 * rho) #fc.CF
    # surface_tension_force Zhang eq(5) using chemical potential acc. Zhang eq(6) mu_phi
    # scale with division by  / fc.CF analog """Body force vector scaled to lattice units."""   
    #zhang_surface_tension_force = st_mu_phi(fc, _phi) * c_first_derivative0(_phi) / (2.0 * rho)
    #zhang_surface_tension_force = st_mu_phi(fc, _phi) * c_first_derivative0(_phi) 
    zhang_surface_tension_force = st_mu_phi(fc, _phi) * zhang_gradient(_phi) 

    fx = zhang_surface_tension_force[0]
    tol = 1e-12
    y = (Yn+2)//2-2
    # First from start
    left_node = first_clear_zero_index(fx, y, tol)
    # Last from end
    right_node = last_clear_zero_index(fx, y, tol)

    zhang_chemicalPotential = st_mu_phi(fc, _phi_n)
    inamuro_chemicalPotential = mu_c(fc, _phi) / (2.0 * rho) #fc.CF

    # only the first x-component of surface tension force acc. Zhang is added to Inamuro eq(5) 
    mod_zhang_surface_tension_force = create_modified_surface_tension_force(zhang_surface_tension_force, 10, 10)

    #Zahng modified force:
    zhang_surface_tension_force = _zhang_fc_phi_n

    return _phi, \
        zhang_chemicalPotential, inamuro_chemicalPotential, \
        inamuro_surface_tension_force, zhang_surface_tension_force, \
        mod_zhang_surface_tension_force


def create_modified_surface_tension_force(zhang_surface_tension_force, 
                                          bottom_thickness=10, 
                                          top_thickness=10):
    """
    Creates a modified version of zhang_surface_tension_force where:
      - Interior → only horizontal (x) component
      - Bottom boundary (first bottom_thickness rows) → vertical (y) component
      - Top boundary (last top_thickness rows) → vertical (y) component
      - Corner regions → 45° diagonal transition so horizontal and vertical
        components meet cleanly without overlap

    Parameters:
    -----------
    zhang_surface_tension_force : np.ndarray
        Shape (2, Xn, Yn) with [fx, fy] components
    bottom_thickness : int, default 10
        Number of rows from bottom to treat as boundary
    top_thickness : int, default 10
        Number of rows from top to treat as boundary

    Returns:
    --------
    modified_force : np.ndarray
        Same shape (2, Xn, Yn), with modified components
    """
    if zhang_surface_tension_force.ndim != 3 or zhang_surface_tension_force.shape[0] != 2:
        raise ValueError("Input must be shape (2, Xn, Yn)")

    shape = zhang_surface_tension_force.shape
    Xn, Yn = shape[1], shape[2]

    # Start with only horizontal component everywhere
    modified = np.zeros_like(zhang_surface_tension_force)
    modified[0] = zhang_surface_tension_force[0]          # fx everywhere initially

    # --------------------
    # Bottom region
    # --------------------
    if bottom_thickness > 0:
        bottom_mask = np.zeros((Xn, Yn), dtype=bool)
        bottom_mask[:, :bottom_thickness] = True

        # Bottom-left 45° diagonal: keep vertical only below y = x
        for x in range(min(bottom_thickness, Xn)):
            for y in range(min(bottom_thickness, Yn)):
                if y >= x:                      # above or on diagonal → no vertical
                    bottom_mask[x, y] = False

        # Bottom-right 45° diagonal: keep vertical only where y < (Xn-1 - x)
        for x in range(max(0, Xn - bottom_thickness), Xn):
            dist_right = Xn - 1 - x
            for y in range(min(bottom_thickness, Yn)):
                if y >= dist_right:
                    bottom_mask[x, y] = False

        # Apply: remove fx, add fy in bottom region
        modified[0][bottom_mask] = 0.0
        modified[1][bottom_mask] = zhang_surface_tension_force[1][bottom_mask]

    # --------------------
    # Top region
    # --------------------
    if top_thickness > 0:
        top_mask = np.zeros((Xn, Yn), dtype=bool)
        top_mask[:, -top_thickness:] = True

        # Top-left 45° diagonal
        for x in range(min(top_thickness, Xn)):
            for y in range(Yn - top_thickness, Yn):
                dist_from_top = Yn - 1 - y
                if dist_from_top >= x:          # farther from top than from left wall
                    top_mask[x, y] = False

        # Top-right 45° diagonal
        for x in range(max(0, Xn - top_thickness), Xn):
            dist_right = Xn - 1 - x
            for y in range(Yn - top_thickness, Yn):
                dist_from_top = Yn - 1 - y
                if dist_from_top >= dist_right:
                    top_mask[x, y] = False

        # Apply: remove fx, add fy in top region
        modified[0][top_mask] = 0.0
        modified[1][top_mask] = zhang_surface_tension_force[1][top_mask]

    return modified


def first_clear_zero_index(arr_2d, y_idx, tol=1e-10):
    # scan along y at fixed x = x_idx
    line = arr_2d[:, y_idx]

    exact = np.where(line == 0.0)[0]
    if exact.size > 0:
        return int(exact[0])

    close = np.where(np.abs(line) <= tol)[0]
    return int(close[0]) if close.size > 0 else None


def last_clear_zero_index(arr_2d, y_idx, tol=1e-10):
    # scan along y at fixed x = x_idx
    line = arr_2d[:, y_idx]
    ...
    close = np.where(np.abs(line) <= tol)[0]
    return int(close[-1]) if close.size > 0 else None
...
PARAMETER_STUB = "__" + ACTIVE_CASE + "__tau_f_" + str(fc.tau_f) + "__Kf_" + str(fc.Kf)
...
if PHI_DISTRIBUTION == "HORIZONTAL":
    # Replace the original _phi initialization with a call to the method
    _phi = init_horizontal_phi(Xn+2, Yn+2, fc.phi_star_G, fc.phi_star_L, height=(Yn+1)/2, xi=5.0)
...
list_zhangChemicalPotential = {}
list_inamuroChemicalPotential = {}

list_BodyForce_0 = {}
list_BodyForce_1 = {}
list_SurfaceTensionForce_0 = {}
list_SurfaceTensionForce_1 = {}
list_NetForce = {}
...
_fi = fi_c(fc, np.zeros_like(u_ckl), F, _phi)
...
epsilon_u_ckl_list = []
# ----------------------------------------------------------------------
#  Planes for the vertical integrals
# ----------------------------------------------------------------------
x_left_idx  = int(0.4 * (Xn - 1))   # 0-based index inside the fluid domain
x_right_idx = int(0.6 * (Xn - 1))
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    # ──────────────────────────────────────────────────────────────────────────────────────────
    # Inamuro functions: 2.Equilibrium , 3.Collision/Relaxation, 4.Streaming
    # ──────────────────────────────────────────────────────────────────────────────────────────
    ...
    _fi_c = fi_c(fc, u_ckl, F, _phi)
    #Inamuro eq(3): calculation of the predicted velocity of the two phase fluid        
    _gi_c = gi_c(fc, u_ckl, rho, iteration)        

    # ──────────────────────────────────────────────────────────────────────────────────────────
    # Inamuro functions: 3.Collision/Relaxation, 4.Streaming (advection)
    # ──────────────────────────────────────────────────────────────────────────────────────────
    #Inamuro eq(2): calculation of the order parameter which distiguishes the two phases
    # === 3. COLLISION + STREAMING (inside fi and gi) ===
    _fi  = fi(fc, _fi, _fi_c)
    _gi = gi(fc, _gi, _gi_c, u_ckl, rho, mu, iteration) 
    ...
    #_fi, _gi = bounceBackTopBottom_conservation_halfway_bounce_back(_fi, _gi, Xn, Yn)
    #_fi, _gi = bounceBackTopBottom_conservation(_fi, _gi, Xn, Yn)
    _fi, _gi = bounceBackTopBottom_conservation(_fi_star, _gi_star, Xn, Yn)
    

    #4.1b. top/bottom conservative bounceback
    #apply_periodic_boundary_conditions(_fi, _gi)    
    #_fi, _gi = bounceBackLeftRight_conservation_halfway_bounce_back(_fi, _gi, Xn, Yn)
    #_fi, _gi = bounceBackLeftRight_conservation(_fi, _gi, Xn, Yn)
    _fi, _gi = bounceBackLeftRight_conservation(_fi_star, _gi_star, Xn, Yn)
    ...
    # ──────────────────────────────    
    phi_after_stream_collide = np.sum(_phi[1:Xn+1, 1:Yn+1])
    ...
    _phi, \
        zhang_chemicalPotential, inamuro_chemicalPotential, \
        inamuro_surface_tension_force, zhang_surface_tension_force, \
        mod_zhang_surface_tension_force = surfaceTensionForces(iteration, fc, _phi, rho)

    u_ckl_star = np.einsum('ia,ijk->ajk', c, _gi) \
        + fc.ADD_BODY_FORCE * body_force \
        + fc.ADD_SURFACE_TENSION_FORCE * zhang_surface_tension_force
        
    if iteration in iterationsOfInterest:
        # Store 2D data (existing)
        list_zhangChemicalPotential[iteration] = zhang_chemicalPotential[:,yc].copy()
        list_inamuroChemicalPotential[iteration] = inamuro_chemicalPotential[:,yc].copy()

        list_BodyForce_0[iteration] = body_force[0][:,yc].copy()
        list_BodyForce_1[iteration] = body_force[1][:,yc].copy()
        list_SurfaceTensionForce_0[iteration] = mod_zhang_surface_tension_force[0][:,yc].copy()
        list_SurfaceTensionForce_1[iteration] = mod_zhang_surface_tension_force[1][:,yc].copy()
    ...
    # Inside main while
    while np.max(epsilon) > fc.epsilon_cutoff and ph_iter < fc.max_ph_iters:
        #p_old = _p0.copy()
        p, h = ph(h, rho, u_ckl_star, iteration)  # Use damped div_u if added above
        delta_p = p - _p0
        p = _p0 + fc.omega_sor * delta_p  # Over-relax
    ...
    if np.max(epsilon) > fc.epsilon_cutoff:
    ...
    if ADD_METRICS and iteration in iterationsOfInterest:
        ...
        du_dx, du_dy = c_first_derivative0(u_ckl_star[0])
        dv_dx, dv_dy = c_first_derivative0(u_ckl_star[1])
        div_u_raw = du_dx + dv_dy
...
plotter.phi_profile(phi_on_plane, f"phi_profile_{iteration}", iteration=iteration)
...
zhangChemicalPotential_center = list_zhangChemicalPotential.popitem()[1]
inamuroChemicalPotential_center = list_inamuroChemicalPotential.popitem()[1]

BodyForce_center_0 = list_BodyForce_0.popitem()[1]
BodyForce_center_1 = list_BodyForce_1.popitem()[1]
SurfaceTensionForce_center_0 = list_SurfaceTensionForce_0.popitem()[1]
SurfaceTensionForce_center_1 = list_SurfaceTensionForce_1.popitem()[1]
...
if ADD_METRICS: 
    ...
    series_labels = ["Body Force", "Zhang Surface-Tension Force"]
    #plotter.plot_bounds_ext(SurfaceTensionForceCollector, "Body and Surface Tension Forces", ax2[0, 3], series_labels)
    label1 = r'$F_g[x]$'
    label2 = r'$F_g[y]$'
    label3 = r'$F_s[x]$'    
    label4 = r'$F_s[y]$'    
    label5 = r'$F_{net}[y]$'
    plotter.phi_x_axis_plot_5(ax2[0, 3], 
        BodyForce_center_0, BodyForce_center_1, 
        SurfaceTensionForce_center_0, SurfaceTensionForce_center_1, 
        NetForce_center, yc, iteration, 
        f"Body, Surface Tension and Net Forces y={yc}", 
    ...
    series_labels = ["-F[i]*Kf*phi*d²phi/dx_a²", "-F[i]*Kf/6*(dphi/dx_a)²", "3*E[i]*phi*c_a[i]*u_a[i]", "E[i]*Kf*Gab(phi)*c_a[i]*c_b[i]"]
    #plotter.plot_momentum_bounds(PhiTerms, "PhiTerms", ax2[1, 2], series_labels)
    ...
    #series_labels = ["_bottom_wall_diff_min","_bottom_wall_diff_max","_top_wall_diff_min","_top_wall_diff_max"]
    label1 = 'norm.-left-x'
    label2 = 'norm.-left-y'
    label3 = 'left-x'
    label4 = 'left-y'
    ...
    iteration, norm_right_x, norm_right_y, right_x, right_y = fcBounds_right[-1]  # or .pop() if you want to remove
    plotter.phi_boundary_forces_vertical(
                norm_right_x, norm_right_y, right_x, right_y,
                label_left_x="norm. Right wall Fx",
                label_left_y="norm. Right wall Fy",
                label_right_x="Right wall Fx",
                label_right_y="Right wall Fy",
                yc=yc,
                iteration=iteration,
