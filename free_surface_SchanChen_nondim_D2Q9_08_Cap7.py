...
CAPILLARY_PROOF = FlowConfig(
    ...
    a=1.0, b=6.7, T=3.5e-2,
    tau_g=1.0,
    tau_f=1.5,   # was 1 — restore proven stable value
    Kf=0.002,    # was 0.5 — restore proven stable value
    Kg = 0.0005,
    omega_sor=1.2,
    epsilon_cutoff=1e-4,
    vf_xi = 6, #4, previously was 6    
    vf_sigma = 0.01,
    vf_theta = 120.0, #60
...
def phi_wall(fc: FlowConfig, _phi: np.ndarray, wall_x: int = None, wall_y: int = None):
    """
    Apply Inamuro simple phi wall boundary condition.
    
    Supports left/right (via wall_x) and bottom/top (via wall_y).
    Works with your indexing: _phi[x, y] with shape (Xn, Yn)
    
    Note: 
        - Left fluid wall  → wall_x = 1
        - Right fluid wall → wall_x = Xn-1
        - Bottom fluid wall→ wall_y = 1
        - Top fluid wall   → wall_y = Yn-1
    """
    if wall_x is None and wall_y is None:
        raise ValueError("You must specify either wall_x or wall_y")

    # Compute S and phi on wall
    ...
    if wall_x is not None:
        ...
        interior = _phi[wall_x + sign, :]        
        mid_phi = (fc.phi_star_G + fc.phi_star_L) / 2

        # interface 
        yc_wall = np.argmin(np.abs(interior - mid_phi))

        # apply phi_wall within ±half_width nodes of interface
        # originally 10, increased to 40 to prevent oscillating wall artifacts
        #decreased to 7 after half_width=40 horrorshow
        half_width = 7
        y_idx = np.arange(len(interior))
        in_band = np.abs(y_idx - yc_wall) <= half_width

        ghost = np.where(in_band, _phi_wall, interior)

        #mask corners
        ghost[0]    = fc.phi_star_L   # bottom corner — liquid  ← originally correct
        ghost[Yn+1] = fc.phi_star_G   # top corner    — gas     ← originally correct

        # Apply on the wall column
        _phi[wall_x, :] = ghost

    # ====================== Horizontal walls (bottom/top) ======================
    elif wall_y is not None:
        # Apply on the wall row
        _phi[:, wall_y] = _phi_wall
...
def Fsv(fc, _phi, _rho):
    ...
    _nabla_rho = c_first_derivative0(_rho)
    _Fsv = sigma * chi(fc, _rho) * (_nabla_rho * _rho)/(delta_rho * rho_avg)
...
PARAMETER_STUB = "__" + ACTIVE_CASE + "__nodes_" + str(DEFAULT_D_ND) + "__tau_f_" + str(fc.tau_f) + "__Kf_" + str(fc.Kf) + "__Theta_" + str(fc.vf_theta)
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    phi_wall(fc, _phi, wall_x=0)
    phi_wall(fc, _phi, wall_x=Xn+1)
    ...
    if iteration in iterationsOfInterest:
        ...
        _fy_val  = np.mean(surface_tension_force[1, 1, yc-5:yc+5])
    ...
    if iteration in iterationsOfInterest:
        ...
        netForce = fc.ADD_BODY_FORCE * body_force + fc.ADD_SURFACE_TENSION_FORCE * surface_tension_force
