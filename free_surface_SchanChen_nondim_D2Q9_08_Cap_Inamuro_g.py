...
class FlowConfig:
    ...
    vf_sigma: float = 0.001 
    '''θ - equilibrium contact angle'''
    vf_theta: int = 0    

    # --- Defaults ---
    xi: float = 5.0                     # gas/liquid interface thickness
    W: float = 4.0                      # Zhang interface thickness
    sigma: float = 0.001                # Zhang surface tension coefficient, A. Wetting behavior of a particle on the liquid–gas interface
    ...
    def g_lattice(self) -> float:
        """Normalised g acc. Inamuro A2"""
        return self.g * self.dx
...
def mu_c(fc, __phi):
    ...
    Uses c_laplacian(_phi) (with the correct 1/(3 dx²) D2Q9 form).
    """
...
def gi(iteration, fc, _gi, _gi_c, u_ckl, rho, mu, _body_force, _zhang_surface_tension_force):
    """
    ...
    # Project _body_force onto each direction i using c_i · F
    dot_cF = (c_x_exp * _body_force[0, :, :][np.newaxis, :, :] + 
            c_y_exp * _body_force[1, :, :][np.newaxis, :, :])   # shape (9, Xn, Yn)
    body_force_term = -3.0 * E_exp * dot_cF
    # Project _body_force onto each direction i using c_i · F
    dot_cCap = (c_x_exp * _zhang_surface_tension_force[0, :, :][np.newaxis, :, :] + 
            c_y_exp * _zhang_surface_tension_force[1, :, :][np.newaxis, :, :])   # shape (9, Xn, Yn)
    zhang_surface_tension_force_term = 3.0 * E_exp * dot_cCap

    # Collision: fully vectorized BGK + forcing
    _gi_old = np.copy(_gi)  # (9, nx, ny)
    _gi_star = _gi_old - (1.0 / fc.tau_g) * (_gi_old - _gi_c) + viscous_term + body_force_term + zhang_surface_tension_force_term
...
def bounceBackTopBottom_conservation0(fi, gi, nx, ny):
    """
    Fully conservative + physically correct bounce-back for top/bottom walls
    Works with D2Q9, ghost layers already filled by mirror/extrapolation
    Arrays are shape (9, nx+2, ny+2)  [assuming periodic in x → +2 in x too]
    """

    # === Bottom wall (y = 1) ===
    # Straight directions - normal bounce-back
    fi[2, 1:nx+1, 1] = fi[4, 1:nx+1, 0]    # N  ← S (from ghost)
    gi[2, 1:nx+1, 1] = gi[4, 1:nx+1, 0]

    # Diagonal directions - CRITICAL: NO ROLL! Pure reflection
    ...
    # === Top wall (y = ny) ===
    fi[4, 1:nx+1, ny] = fi[2, 1:nx+1, ny+1]
    fi[7, 1:nx+1, ny] = fi[5, 1:nx+1, ny+1]
    fi[8, 1:nx+1, ny] = fi[6, 1:nx+1, ny+1]
...
def calc_phi_w(phi_f, thetaCap, dx):
    """
    ...
    """
    discriminant = (thetaCap + 1.0/dx)**2 - 4.0 * thetaCap * phi_f / dx
    # Ensure discriminant is non-negative (numerical safety)
    discriminant = np.maximum(discriminant, 0.0)
    
    __phi_w = (thetaCap + 1.0/dx - np.sqrt(discriminant)) / (2.0 * thetaCap)
...
def denormalizePhi1(fc, phi_n):
    phi_dn = fc.phi_star_G + phi_n * (fc.phi_star_L - fc.phi_star_G)
    return phi_dn

def st_beta(fc):
    beta = (12 * fc.sigma) / fc.W
    return beta

def st_kappa(fc):
    kappa = (3 * fc.sigma * fc.W) / 2.0    
    return kappa

def compContactAngle(fc):
    cos_theta = np.cos(np.deg2rad(fc.vf_theta))
    _compContactAngle = -np.sqrt(2*st_beta(fc)/st_kappa(fc)) * cos_theta
...
def st_mu_phi(fc, _phi):
    ...
    """
    lap = c_laplacian(_phi)
    term1 = 4 * st_beta(fc)
    term2 = _phi*(_phi - 1)
    term3 = (_phi - 0.5)
    term4 = st_kappa(fc) * lap
    _mu_phi = term1 * term2 * term3 - term4
    return _mu_phi

# returns body_force, inamuro_surface_tension_force, zhang_surface_tension_force  
def calculateForces(fc, rho, _phi, _fi, _gi):
    ...
    # 1. body mass force
    #body_force = A*bodyForce(fc.F_lattice, rho)*n_dt / (2.0*fc.CF)
    body_force = A*bodyForce(fc.F_lattice, rho)*n_dt
    body_force1 = A*bodyForce(fc.F_lattice, rho)*n_dt / (2.0*rho)
    ...
    # === Bottom wall (y=1) ===
    _phi_f_n_bottom = _phi_n[1:Xn+1, 1]                    # fluid node inside
    _phi_w_n_bottom = calc_phi_w(_phi_f_n_bottom, thetaCap, n_dx)

    # === Left wall (x=1) ===
    _phi_f_n_left = _phi_n[1, 1:Yn+1]
    _phi_w_n_left = calc_phi_w(_phi_f_n_left, thetaCap, n_dx)

    # === Right wall (x=Xn) ===
    _phi_f_n_right = _phi_n[Xn, 1:Yn+1]
    ...
    # ← NO assignment to top wall (y=Yn) — left as gas/neutral
    # enforce no fluid adhesion at top.
    if False:
        _phi[:, Yn:Yn+2] = fc.phi_star_G
        # Top-left corner: average from left wall and inner top
        _phi[1, Yn] = 0.5 * (_phi[1, Yn-1] + _phi[2, Yn])
        # Top-right similar
        _phi[Xn, Yn] = 0.5 * (_phi[Xn, Yn-1] + _phi[Xn-1, Yn])
    ...
    # top wall -> _phi in ghost layer == 
    # Zero normal gradient for distributions at top (open boundary)
    _fi[:, :, Yn+1] = _fi[:, :, Yn]      # Copy from interior to ghost
    _gi[:, :, Yn+1] = _gi[:, :, Yn]

    # also extrapolate _phi for consistent gradients
    _phi[:, Yn+1] = _phi[:, Yn]         # Zero dphi/dy at top  
    ...
    inamuro_surface_tension_force = mu_c(fc, _phi) * c_first_derivative(_phi) 
    # surface_tension_force Zhang eq(5) using chemical potential acc. Zhang eq(6) mu_phi
    # scale with division by  / fc.CF analog """Body force vector scaled to lattice units."""   
    zhang_term1 = st_mu_phi(fc, _phi)
    zhang_term2 = c_first_derivative(_phi)
    zhang_surface_tension_force = zhang_term1 * zhang_term2 
    ...
    return body_force, inamuro_surface_tension_force, zhang_surface_tension_force              
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    body_force, inamuro_surface_tension_force, zhang_surface_tension_force  = calculateForces(fc, rho, _phi, _fi, _gi)   
    _body_force = fc.ADD_BODY_FORCE * body_force 
    _zhang_surface_tension_force = fc.ADD_SURFACE_TENSION_FORCE * zhang_surface_tension_force


    #Inamuro eq(2): calculation of the order parameter which distiguishes the two phases
    # === 3. COLLISION + STREAMING (inside fi and gi) ===
    _fi = fi(fc, _fi, _fi_c)
    #Inamuro eq(3): calculation of a predicted velocity of the two-phase ﬂuid without a pressure gradient
    _gi = gi(iteration, fc, _gi, _gi_c, u_ckl, rho, mu, _body_force, _zhang_surface_tension_force) 
    ...
    #_phi = gaussian_filter(_phi, sigma=0.5)   # sigma=0.5–1.0 is usually enough
    ...
    #moved BEFORE equilibrium methods
    ...
    if iteration % 500 == 0 or iteration == 0:  # Check early and periodically
        # Pick a point near the left wall (e.g., fluid node next to wall)
        i = 2   # x=2 (just inside fluid from left wall x=1)
        j = Yn // 2  # middle height

        # Raw phi (before any normalisation)
        phi_raw = _phi[i, j]
        ...
        # Compute gradients BEFORE normalisation (on original _phi)
        grad_x_raw, grad_y_raw = c_first_derivative(_phi, n_dx, n_dy)
        grad_x_before = grad_x_raw[i, j]
        grad_y_before = grad_y_raw[i, j]

        # Compute gradients AFTER denormalisation would be applied (but on same field)
        # Since denormalisation is linear and monotonic, signs must match
        grad_x_after, grad_y_after = c_first_derivative(_phi, n_dx, n_dy)  # same as before
        # But let's simulate if we had a normalised field and gradients
        delta_phi = _phi_max - _phi_min
        grad_x_sim = grad_x_before * delta_phi  # scaling factor
        grad_y_sim = grad_y_before * delta_phi

        print("=== Normalisation Gradient Sign Check ===")
        print(f"ϕ raw at [{i},{j}]: {phi_raw:.6f}")
        print(f"ϕ normalised: {phi_n:.6f}")
        print(f"∇ϕ_x before scaling: {grad_x_before:.6e}  → after: {grad_x_sim:.6e}  (sign same: {np.sign(grad_x_before) == np.sign(grad_x_sim)})")
        print(f"∇ϕ_y before scaling: {grad_y_before:.6e}  → after: {grad_y_sim:.6e}  (sign same: {np.sign(grad_y_before) == np.sign(grad_y_sim)})")       
        print(f"zhang_surface_tension_force x-direction: {zhang_surface_tension_force[0,1,25]:.6e}")
        print(f"zhang_surface_tension_force y-direction: {zhang_surface_tension_force[1,1,25]:.6e}")
