...
def c_first_derivative(lamda, n_dx=1.0, n_dy=1.0):
    # Pad for boundaries
    padded = np.pad(lamda, 2, mode='edge')
    # 4th-order central diff
    dx = ( -padded[4:, 2:-2] + 8*padded[3:-1, 2:-2] - 8*padded[1:-3, 2:-2] + padded[:-4, 2:-2] ) / (12 * n_dx)
    dy = ( -padded[2:-2, 4:] + 8*padded[2:-2, 3:-1] - 8*padded[2:-2, 1:-3] + padded[2:-2, :-4] ) / (12 * n_dx)
    return dx, dy
...
def gi(fc, _gi, _gi_c, u_ckl, rho, mu, iteration):
    ...
    if use_central_difference_scheme:
        ...
        d_sigma_xx_dx = c_first_derivative(sigma_xx, n_dx, n_dy)[0]
        d_sigma_xy_dy = c_first_derivative(sigma_xy, n_dx, n_dy)[1]
        div_sigma_x = d_sigma_xx_dx + d_sigma_xy_dy

        d_sigma_yx_dx = c_first_derivative(sigma_yx, n_dx, n_dy)[0]
        d_sigma_yy_dy = c_first_derivative(sigma_yy, n_dx, n_dy)[1]
        div_sigma_y = d_sigma_yx_dx + d_sigma_yy_dy        
...
def bounceBackTopBottom_conservation1(fi, gi, nx, ny):
    """
    Conservative bounce-back for top/bottom walls using ghost layers.
    Arrays shape: (9, nx+2, ny+2)
    Fluid domain: x=1 to nx, y=1 to ny
    Ghost layers: x=0, x=nx+1, y=0, y=ny+1

    Bottom wall (y=1): full no-slip (solid wall)
    Top    wall (y=ny): free-slip (open to gas/air, no artificial adhesion)
    """

    # === Bottom wall (y=1) - FULL NO-SLIP using ghost y=0 ===
    # Straight directions
    fi[2, 1:nx+1, 1] = fi[4, 1:nx+1, 0]    # N ← S (from ghost)
    gi[2, 1:nx+1, 1] = gi[4, 1:nx+1, 0]

    # Diagonals — pure reflection (no roll)
    fi[5, 1:nx+1, 1] = fi[7, 1:nx+1, 0]    # NE ← SW
    fi[6, 1:nx+1, 1] = fi[8, 1:nx+1, 0]    # NW ← SE

    gi[5, 1:nx+1, 1] = gi[7, 1:nx+1, 0]
    gi[6, 1:nx+1, 1] = gi[8, 1:nx+1, 0]

    # === Top wall (y=ny) - FREE-SLIP using specular reflection on ghost y=ny+1 ===
    # This allows tangential flow (no shear stress) while bouncing normal component
    # Equivalent to mirroring populations across the top boundary

    # Straight normal: bounce back (reverse direction)
    fi[4, 1:nx+1, ny] = fi[2, 1:nx+1, ny+1]   # S ← N (from ghost)
    gi[4, 1:nx+1, ny] = gi[2, 1:nx+1, ny+1]

    # Diagonals: specular reflection (swap left/right component)
    fi[7, 1:nx+1, ny] = fi[5, 1:nx+1, ny+1]   # SW ← NE
    fi[8, 1:nx+1, ny] = fi[6, 1:nx+1, ny+1]   # SE ← NW

    gi[7, 1:nx+1, ny] = gi[5, 1:nx+1, ny+1]
    gi[8, 1:nx+1, ny] = gi[6, 1:nx+1, ny+1]
...
def bounceBackLeftRight_conservation(fi, gi, nx, ny):
    """
    Conservative no-slip bounce-back for left/right walls using on-node reflection.
    Does NOT depend on ghost node values → safe even if ghosts not filled in x.
    Arrays shape: (9, nx+2, ny+2), walls at x=1 and x=nx
    """

    # === Left wall (x=1) ===
    fi[1, 1, 1:ny+1] = fi[3, 1, 1:ny+1]    # E ← W (local)
    fi[5, 1, 1:ny+1] = fi[7, 1, 1:ny+1]    # NE ← SW
    fi[8, 1, 1:ny+1] = fi[6, 1, 1:ny+1]    # SE ← NW

    gi[1, 1, 1:ny+1] = gi[3, 1, 1:ny+1]
    gi[5, 1, 1:ny+1] = gi[7, 1, 1:ny+1]
    gi[8, 1, 1:ny+1] = gi[6, 1, 1:ny+1]

    # === Right wall (x=nx) ===
    fi[3, nx, 1:ny+1] = fi[1, nx, 1:ny+1]    # W ← E
    fi[7, nx, 1:ny+1] = fi[5, nx, 1:ny+1]    # SW ← NE
    fi[6, nx, 1:ny+1] = fi[8, nx, 1:ny+1]    # NW ← SE

    gi[3, nx, 1:ny+1] = gi[1, nx, 1:ny+1]
    gi[7, nx, 1:ny+1] = gi[5, nx, 1:ny+1]
    gi[6, nx, 1:ny+1] = gi[8, nx, 1:ny+1]
...
def calc_phi_w_2ndorder(phi_f1, phi_f2, thetaCap, n_dx, rootOption="-"):
    """
    Compute wet-node wall value __phi_w using Zhang's Eq. (10)
...
def compContactAngle(fc):
    cos_theta = np.cos(np.deg2rad(fc.vf_theta))
...
def st_mu_phi(fc, _phi):
    ...
    """
    term1 = 4 * fc.vf_beta * _phi*(_phi - 1) * (_phi - 0.5)
    term2 = fc.vf_kappa
    term3 = c_laplacian(_phi)
    _mu_phi =  term1 -  term2 * term3
    return _mu_phi


def n0(_phi):
    # 1) Compute gradient of phi: ∇phi
    dphi_dy, dphi_dx = np.gradient(_phi, n_dy, n_dx)  # note: (y, x) order
...
def n1(_phi):
    # 1) Compute gradient of phi: ∇phi
    dphi_dy, dphi_dx = c_first_derivative(_phi)  # note: (y, x) order
...
def surfaceTensionForces(iteration, fc, order, _phi, rho):
    ...
    if order==1: # first order solution
        # === Top wall (y=1) ===
        _phi_f_n_top = _phi_n[1:Xn+1, Yn-1]                    # fluid node inside
        _phi_w_n_top = calc_phi_w(_phi_f_n_top, thetaCap, n_dx)    

        # === Bottom wall (y=1) ===
        _phi_f_n_bottom = _phi_n[1:Xn+1, 2]                    # fluid node inside
        _phi_w_n_bottom = calc_phi_w(_phi_f_n_bottom, thetaCap, n_dx)

        # === Left wall (x=1) ===
        _phi_f_n_left = _phi_n[2, 1:Yn+1]
        _phi_w_n_left = calc_phi_w(_phi_f_n_left, thetaCap, n_dx)

        # === Right wall (x=Xn) ===
        _phi_f_n_right = _phi_n[Xn-1, 1:Yn+1]
        _phi_w_n_right = calc_phi_w(_phi_f_n_right, thetaCap, n_dx)
    elif order == 2:
        # === Top wall (y=1) ===
        _phi_f_n_top1 = _phi_n[1:Xn+1, Yn-1]                    # 1. fluid node inside
        _phi_f_n_top2 = _phi_n[1:Xn+1, Yn-2]                    # 2. fluid node inside
        _phi_w_n_top = calc_phi_w_2ndorder(_phi_f_n_top1, _phi_f_n_top2, thetaCap, n_dx, "-")    

        # === Bottom wall (y=1) ===
        _phi_f_n_bottom1 = _phi_n[1:Xn+1, 2]                    # 1. fluid node inside
        _phi_f_n_bottom2 = _phi_n[1:Xn+1, 3]                    # 2. fluid node inside
        _phi_w_n_bottom = calc_phi_w_2ndorder(_phi_f_n_bottom1, _phi_f_n_bottom2, thetaCap, n_dx, "-")    

        # === Left wall (x=1) ===
        _phi_f_n_left1 = _phi_n[2, 1:Yn+1]                    # 1. fluid node inside
        _phi_f_n_left2 = _phi_n[3, 1:Yn+1]                    # 2. fluid node inside
        _phi_w_n_left = calc_phi_w_2ndorder(_phi_f_n_left1, _phi_f_n_left2, thetaCap, n_dx, "-")    

        # === Right wall (x=Xn) ===
        _phi_f_n_right1 = _phi_n[Xn-1, 1:Yn+1]                    # 1. fluid node inside
        _phi_f_n_right2 = _phi_n[Xn-2, 1:Yn+1]                    # 2. fluid node inside
        _phi_w_n_right = calc_phi_w_2ndorder(_phi_f_n_right1, _phi_f_n_right2, thetaCap, n_dx, "-")
    else:
        raise ValueError("order of phi-derivitive at wall must be specified")
    ...
    _phi[1:Xn+1, 1]  = _phi_w_bottom   # bottom
    #_phi[1, 1:Yn+1]   = _phi_w_left     # left
    #_phi[Xn, 1:Yn+1]  = _phi_w_right    # right

    # Left wall: extend wetting influence to x=-1 (double ghost)
    # -> _phi[-1, 1:Yn+1] = _phi_w_left  # extra ghost layer at x=-1
    _phi[1, 1:Yn+1] = _phi_w_left   # current x=0
    # -> _phi[1, 1:Yn+1]   = _phi_w_left     # left

    # Right wall
    _phi[Xn, 1:Yn+1] = _phi_w_right  # current x=Xn+1  
    # -> _phi[Xn, 1:Yn+1]  = _phi_w_right    # right      

    # === Corner averaging for solid corners only (bottom-left and bottom-right) ===
    _phi[1, 1]   = 0.5 * (_phi_w_bottom[0]   + _phi_w_left[0])     # bottom-left
    _phi[Xn, 1]  = 0.5 * (_phi_w_bottom[-1]  + _phi_w_right[0])    # bottom-right


    # ──────────────────────────────────────────────────────────────
    # Top boundary: truly neutral open channel (zero-gradient extrapolation)
    # ──────────────────────────────────────────────────────────────
    # Extrapolate from top fluid row (zero normal gradient, no preference for gas/liquid)
    _phi[:, Yn+1] = _phi[:, Yn]   # copy top fluid row → neutral

    # Zero normal gradient for distributions (open boundary condition)
    _fi[:, :, Yn+1] = _fi[:, :, Yn]      # Copy from interior to ghost
    _gi[:, :, Yn+1] = _gi[:, :, Yn]
    # ──────────────────────────────────────────────────────────────
    #         
    ...
    if abs(phi_total_after - phi_old_total) > 1e-8:  # avoid div-by-zero or noise
    ...
    if iteration % 100 == 0:
        debug_log('ITER', 'Iter %d: Wall dPhi bottom min/max = %.3e / %.3e',
                iteration, np.min(delta_phi[1:Xn+1, 1]), np.max(delta_phi[1:Xn+1, 1]))
        debug_log('ITER', 'Iter %d: Wall dPhi left   min/max = %.3e / %.3e',
                iteration, np.min(delta_phi[1, 1:Yn+1]), np.max(delta_phi[1, 1:Yn+1]))
        debug_log('ITER', 'Iter %d: Wall dPhi right  min/max = %.3e / %.3e',
                iteration, np.min(delta_phi[Xn, 1:Yn+1]), np.max(delta_phi[Xn, 1:Yn+1]))

    if ADD_METRICS and (iteration == 0 or iteration == (fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0):
        DeltaPhiW.append((iteration,
                        np.min(delta_phi[1:Xn+1, 1]), np.max(delta_phi[1:Xn+1, 1]),   # bottom
                        np.min(delta_phi[1, 1:Yn+1]), np.max(delta_phi[1, 1:Yn+1]),   # left
                        np.min(delta_phi[Xn, 1:Yn+1]), np.max(delta_phi[Xn, 1:Yn+1])))  # right
    # -------------------------------------------------------------------------------

    # ──────────────────────────────────────────────────────────────
    # NEW: MASS DIAGNOSTIC after wetting (this is where we expect injection)
    phi_new_total = np.sum(_phi[1:Xn+1, 1:Yn+1])

    if iteration % 500 == 0 or iteration < 50:  # print every 50 steps + first 50
        delta = phi_new_total - phi_old_total
        print(f"Iter {iteration:6d} | Δmass = {delta:+.8f}  | total_phi = {phi_new_total:.4f}")

        # Optional: how much wetting changed it
        # (you can remove this line later if not needed)
        print(f"          | Wetting added ≈ {phi_new_total - phi_old_total:+.8f}")  # rough estimate
    # ──────────────────────────────────────────────────────────────    
    ...
    inamuro_surface_tension_force = mu_c(fc, _phi) * c_first_derivative(_phi) / (2.0 * rho) #fc.CF
    # surface_tension_force Zhang eq(5) using chemical potential acc. Zhang eq(6) mu_phi
    # scale with division by  / fc.CF analog """Body force vector scaled to lattice units."""   
    zhang_surface_tension_force = st_mu_phi(fc, _phi) * c_first_derivative(_phi) / (2.0 * rho) #fc.CF
    chemicalPotential = st_mu_phi(fc, _phi) / (2.0 * rho)

    #TEST
    if False:
        zhang_surface_tension_force1 = st_mu_phi(fc, _phi) * c_first_derivative(_phi) / (2.0 * rho) #fc.CF
        fc.vf_W = 6
        fc.vf_sigma = 0.72
        zhang_surface_tension_force2= st_mu_phi(fc, _phi) * c_first_derivative(_phi) / (2.0 * rho) #fc.CF

    return _phi, chemicalPotential, inamuro_surface_tension_force, zhang_surface_tension_force
...
list_phiw = {}
list_dphi = {}
list_ChemicalPotential = {}

list_BodyForce = {}
list_SurfaceTensionForce = {}
list_NetForce = {}

yc = (Yn+2)//2 -2
surface_tension_solution_order = 2
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    # 4. Apply conservative bounce-back (no rolls!)
    _fi, _gi = bounceBackTopBottom_conservation1(_fi, _gi, Xn, Yn)
    ...
    if iteration in iterationsOfInterest:
        ...
        list_dphi[iteration] = c_first_derivative(_phi)[0][:,yc].copy()
    ...
    _phi, chemicalPotential, inamuro_surface_tension_force, zhang_surface_tension_force = surfaceTensionForces(iteration, fc, surface_tension_solution_order, _phi, rho)

    if iteration == 0 or iteration==(fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0:
        if ADD_METRICS:
            _body_force = np.max(np.abs(body_force))
            _InamuroSurfaceTensionForce = np.max(np.abs(inamuro_surface_tension_force))
            _ZhangSurfaceTensionForce = np.max(np.abs(zhang_surface_tension_force))
            SurfaceTensionForceCollector.append((iteration, _body_force, _ZhangSurfaceTensionForce))        
    ...
    if iteration in iterationsOfInterest:
        # Store 2D data (existing)
        list_phiw[iteration] = _phi[:,yc].copy()
        list_ChemicalPotential[iteration] = chemicalPotential[:,yc].copy()
        list_BodyForce[iteration] = body_force[1][:,yc].copy()
        list_SurfaceTensionForce[iteration] = zhang_surface_tension_force[1][:,yc].copy()
...
# phi_
filtered_phi_dict = plotter.filter_u_ckl_fullrange(list_phi, iterationsOfInterest)
filtered_phi_list = list(filtered_phi_dict.values())

filtered_phiw_dict = plotter.filter_u_ckl_fullrange(list_phiw, iterationsOfInterest)
filtered_phiw_list = list(filtered_phiw_dict.values())

filtered_dphi_dict = plotter.filter_u_ckl_fullrange(list_dphi, iterationsOfInterest)
filtered_dphi_list = list(filtered_dphi_dict.values())

filtered_ChemicalPotential_dict = plotter.filter_u_ckl_fullrange(list_ChemicalPotential, iterationsOfInterest)
filtered_ChemicalPotential_list = list(filtered_ChemicalPotential_dict.values())

filtered_BodyForce_dict = plotter.filter_u_ckl_fullrange(list_BodyForce, iterationsOfInterest)
filtered_BodyForce_list = list(filtered_BodyForce_dict.values())

filtered_SurfaceTensionForce_dict = plotter.filter_u_ckl_fullrange(list_SurfaceTensionForce, iterationsOfInterest)
filtered_SurfaceTensionForce_list = list(filtered_SurfaceTensionForce_dict.values())

filtered_NetForce_dict = plotter.filter_u_ckl_fullrange(list_NetForce, iterationsOfInterest)
filtered_NetForce_list = list(filtered_NetForce_dict.values())
...
phi_center = filtered_phi_list[-1][:]
phi_w_center = filtered_phiw_list[-1][:]
phid_center = filtered_dphi_list[-1][:]
ChemicalPotential_center = filtered_ChemicalPotential_list[-1][:]
BodyForce_center = filtered_BodyForce_list[-1][:]
SurfaceTensionForce_center = filtered_SurfaceTensionForce_list[-1][:]
NetForce_center = filtered_NetForce_list[-1][:]


label1 = r'$\phi$'
label2 = r'$\partial \phi$'
plotter.phi_x_axis_plot_2(ax1[3, 0], phi_center, phid_center, yc, iteration, f"_phi + _phid distribution y={yc}", label1, label2)
#plotter.phi_x_axis_plot(ax1[3, 1], filtered_dphi_list[-1][:], yc, iteration, f"_dphi distribution y={yc}")
label=r'$\mu_\phi$'
plotter.phi_x_axis_plot_1(ax1[3, 1], ChemicalPotential_center, yc, iteration, f"ChemicalPotential distribution y={yc}", label)
...
if ADD_METRICS: 
    ...
    label2 = r'$F_s$'    
    label3 = r'$F_net$'    
    plotter.phi_x_axis_plot_3(ax2[0, 3], BodyForce_center, SurfaceTensionForce_center, NetForce_center, yc, iteration, f"Body, Surface Tension and Net Forces y={yc}", label1, label2, label3)
    ...
    series_labels = ["min(_phi-_phi_w)","max(_phi-_phi_w)"]
    #series_labels = ["Bnon","Blat"]
    #plotter.plot_bounds_ext(DivU_max, "DivU_max", ax2[2, 2])
    plotter.plot_bounds_ext(DeltaPhiW, "Delta (_phi-_phi_w) on bottom wall boundary nodes", ax2[2, 2], series_labels)
    #plotter.plot_bounds_ext(BondNumber, "Bond no. in non-dim (Bnon) and lattice (Blat) units", ax2[2, 2], series_labels)
