...
NODES = int(os.getenv("NODES", 1))  # Default to 1 if not set
_L = int(os.getenv("L", 1))       # Default to 1 if not set
Xn = int(os.getenv("Xn", NODES * 96 * 50 * _L))       # Default to 96 if not set
Yn = int(os.getenv("Yn", D_nd))       # Default to 50 if not set
...
def collect_u_ckl2(__u_ckl, _u_ckl_full_range):
    _sendBuf_roh = np.ascontiguousarray(__u_ckl[1:-1, :], dtype=np.float32)
    if rank == 0:
        assert _u_ckl_full_range.flags['C_CONTIGUOUS']
        expected_shape = (Xn, Yn+2) 
        assert _u_ckl_full_range[1:-1, :].shape == expected_shape

        comm.Gather(_sendBuf_roh, _u_ckl_full_range[1:-1, :], root=0)
    else:
        comm.Gather(_sendBuf_roh, None, root=0)
...
if rank == 0:
    ...
    _u_ckl_full_range = np.zeros((Xn+2, Yn+2), dtype=np.float32)
    _u_ckl_full_range = np.ascontiguousarray(_roh_full_range, dtype=np.float32)    

TOTAL_ITERATION = 10000000 #12010
...
while iteration < TOTAL_ITERATION:
    ...
    #7.1 store velocity amplitudes/density
    ...
    # 7.2 Store only the last u_ckl (no need for list_avg_velocities for multiple iterations)
    last_den = roh
    last_u_ckl = u_ckl  # Store the current u_ckl as the last one    

    # Test convergence (only on last rank)
    converged = False
    if rank == size - 1 and iteration > 5000:
        TEST = np.abs(u_ckl[0, -2, Yn//2] - u_ckl_mid_outlet)
        if TEST < convergence_threshold:
            converged = True
            print(f"Rank {rank} BREAKING at iteration: {iteration}")
            print(f"u_ckl_mid_outlet: {u_ckl_mid_outlet}")
            print(f"u_ckl[0, -2, Yn//2]: {u_ckl[0, -2, Yn//2]}")
            print(f"CRITERION: {TEST} < {convergence_threshold}")

    # Update u_ckl_mid_outlet (only on last rank)
    if rank == size - 1:
        u_ckl_mid_outlet = u_ckl[0, -2, Yn//2]

    # Synchronize convergence decision
    converged = comm.bcast(converged, root=size-1)       
...
_u_ckl_full_range_last = collect_u_ckl2(last_u_ckl, _u_ckl_full_range)

# Collect the last u_ckl for the velocity map
#_u_ckl_velocity_map = collect_u_ckl_for_velocity_map(nlocal, u_ckl)
...
if rank == 0:
    ...
    # [Plotting setup remains unchanged]
    aspect_ratio = Xn / Yn
    top_row_height = 1
    bottom_row_height = 1
    ...
    # Row 0, Col 0: Amplitude plot (unchanged)
    sectionPosition = Xn-1
    #U_max = np.max(last_u_ckl[sectionPosition, 1:Yn+1]) #filtered_u_ckl_list[-1][sectionPosition, 1:Yn+1])
    #aufbau: u_ckl[0, -2, Yn//2]
    U_max = np.max(last_u_ckl[0,sectionPosition, 1:Yn+1]) #filtered_u_ckl_list[-1][sectionPosition, 1:Yn+1])
    ...
    velocity_map(ax[0, 1], _u_ckl_full_range_last[1:-1, 1:Yn+1])

    # Row 1, Col 0: Density profile (unchanged)
    density_profile(ax[1, 0], _roh_full_range, Xn, Yn)

    # Row 1, Col 1: Density map (unchanged)
    density_map(ax[1, 1], _roh_full_range, rho_out, rho_in)

    # [Remaining plotting and file saving remains unchanged]
    ...
    path = f"Milestone5-PoiseuilleFlow23-parallel-{Yn}x{Xn}-nodes{NODES}-L{_L}-iteration_{iteration}_plot.png"
    plt.savefig(path)
    plt.close()

    with open(f"runtime-grid-{Yn}x{Xn}-nodes{NODES}-L{_L}.txt", "w") as f:
        f.write(f"Simulation with {NODES} nodes, grid {Yn}x{Xn}; Run-time: {diff:.1f} s; u_ckl_mid_outlet: {u_ckl_mid_outlet}\n")