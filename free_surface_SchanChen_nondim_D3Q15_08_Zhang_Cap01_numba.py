...
def zhang_interfacial_tension_check(fc, __phi, iteration, check_every=10):
    ...
    print(f"[eq49 check] iter={iteration}  x_slice={x_slice}, y_slice={y_slice}  vf_sigma={fc.vf_sigma:.6f}  "
...
### ------- numba conversion -------
@njit(parallel=True, fastmath=True)
def Fi_kernel(u_ckl, combined_force, stack_grad_rho, c, E, tau_f, Cs2, Cs4, out, Xn2, Yn2, Zn2, Q):
    term1 = 1.0 - 1.0 / (2.0 * tau_f)
    for x in prange(Xn2):
        for y in range(Yn2):
            for z in range(Zn2):
                u0 = u_ckl[0, x, y, z]; u1 = u_ckl[1, x, y, z]; u2 = u_ckl[2, x, y, z]
                u_sq = u0*u0 + u1*u1 + u2*u2
                fx = combined_force[0, x, y, z]; fy = combined_force[1, x, y, z]; fz = combined_force[2, x, y, z]
                gx = stack_grad_rho[0, x, y, z]; gy = stack_grad_rho[1, x, y, z]; gz = stack_grad_rho[2, x, y, z]
                for i in range(Q):
                    ei_dot_u = c[i,0]*u0 + c[i,1]*u1 + c[i,2]*u2
                    s0 = 1.0 + ei_dot_u/Cs2 + ei_dot_u*ei_dot_u/(2.0*Cs4) - u_sq/(2.0*Cs2)
                    s_i = E[i] * s0
                    ex = c[i,0]-u0; ey = c[i,1]-u1; ez = c[i,2]-u2
                    out[i, x, y, z] = term1 * (
                        ex*(fx*s_i + (s_i-E[i])*gx) +
                        ey*(fy*s_i + (s_i-E[i])*gy) +
                        ez*(fz*s_i + (s_i-E[i])*gz)
                    )
    return out

def Fi(fc, Fs, G, u_ckl, rho, out):
    combined_force = Fs * fc.ADD_SURFACE_TENSION_FORCE + G * fc.ADD_BODY_FORCE
    drho_dx, drho_dy, drho_dz = zhang_gradient(fc, rho * Cs2, n_dx, n_dy, n_dz)
    stack_grad_rho = np.stack([drho_dx, drho_dy, drho_dz], axis=0)
    Xn2, Yn2, Zn2 = u_ckl.shape[1], u_ckl.shape[2], u_ckl.shape[3]
    return Fi_kernel(u_ckl, combined_force, stack_grad_rho, c, E, fc.tau_f, Cs2, Cs4, out, Xn2, Yn2, Zn2, fc.velocitySetSize)  
### ------- numba conversion -------
...
### ------- numba conversion ------------
@njit(parallel=True, fastmath=True)
def zfi_c_kernel(u, rho, p, c, E, out, Xn2, Yn2, Zn2, Q):
    for x in prange(Xn2):
        for y in range(Yn2):
            for z in range(Zn2):
                u0 = u[0, x, y, z]
                ...
                for i in range(Q):
                    ...
                    out[i, x, y, z] = E[i] * (pp + r*c_dot_u + r*1.5*c_dot_u*c_dot_u - r*0.5*u_dot_u)
    return out
...
def print_top_layers(_phi_array, num_nodes=4, num_layers=2, y_slice=0):
    ...
    for i in range(num_layers):
        ...
        for x in range(num_nodes):
            print(f"{_phi_array[x, y_slice, layer_idx]:12.8f}", end="   ")
        print()
...
def m_vector(fc, wall):
    ...
    if wall == 'bottom':
        m_bottom = np.array([0, 0, 1])
        return m_bottom
    elif wall == 'top':
        m_top = np.array([0, 0, -1])
        return m_top
    elif wall == 'left':
        n = np.array([1, 0, 0])
        t = np.array([0, 0, 1])
...
def zhang_fc(fc, __phi):
    ...
    _phi : np.ndarray
        Phase field, shape (Xn+2, Yn+2, Zn+2)

    Returns
    -------
    _fc : np.ndarray
        Force array, shape (3, Xn+2, Yn+2, Zn+2)
    """
    dphidx, dphidy, dphidz = zhang_gradient(fc, __phi)
