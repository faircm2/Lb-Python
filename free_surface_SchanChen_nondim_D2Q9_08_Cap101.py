...
CAPILLARY_PROOF = FlowConfig(
    ...
    vf_sigma = 0.01, #0.072
    vf_theta = 60.0, #60
    vf_capillaryForceMultiplier=20,
    MULTIPLES=1,
...
def ph(hn, _rho, u_ckl_star, iteration, n_dx=1.0, n_dy=1.0):
    ...
    # Neumann BC: dp/dx = 0 at solid left/right walls
...
def set_solid_nodes(iteration, fc, _phi):
    ...
    tol = 1e-5   # was 1e-6 — too tight given mass conservation rescaling
...
while iteration < fc.TOTAL_ITERATIONS:
    ...
    # Always apply Zhang phi_s BC
    __phi, _phi_n = set_solid_nodes(iteration, fc, _phi)
    _phi = __phi    

    # Inamuro bulk capillary force: F = -φ ∇μ_c
    # __phi already has Zhang Eq.(32) ghost cells set correctly by set_solid_nodes above
    if fc.ADD_SURFACE_TENSION_FORCE:
        # Capillary force — runs every iteration
        _mu_c = chemical_potential_Inamuro(fc, __phi)          # (Xn+2, Yn+2)
        dmu_dx, dmu_dy = c_first_derivative0(_mu_c, n_dx, n_dy)
        F_cap = np.array([-__phi * dmu_dx, -__phi * dmu_dy])   # (2, Xn+2, Yn+2)
        _capillary_force = A * F_cap * n_dt / (2.0 * rho)       # same scaling as body_force
    else:
        _capillary_force = np.zeros((2, Xn+2, Yn+2))
