...
def gi(_gi, _gi_c, u_ckl, rho, mu):
    ...
    for i in range(9):
        ...
        c_dot_u = np.einsum('i,ikl->kl', c[i], u_ckl)
        c_dot_u_c = c[i, :, None, None] * c_dot_u[None, :, :]  
...
def ForceDensity(F_lattice, rho):
    _force = rho * F_lattice[:, None, None]  # shape (2,202,52)
...
plot_u_ckl_bounds(u_bounds, "u_ckl_bounds_Guo")
