...
while iteration < TOTAL_ITERATIONS:
    ...
    while max_eps > epsilon_cutoff and ph_iter < max_ph_iters:
        ...
        if len(eps_hist) == eps_hist.maxlen:
            trend = np.polyfit(range(len(eps_hist)), eps_hist, 1)[0]
            if abs(trend) < 1e-8:
                break

        if ph_iter % 10 == 0:
            epsilon_cutoff = max(epsilon_cutoff * 0.99, epsilon_cutoff_min)

    # --- log at the end of inner loop ---
    ...
    if max_eps > epsilon_cutoff:
        debug_log(
            'ITER',
            'ph stalled at global %d, eps=%.3e, using approx p',
            iteration, max_eps
        )
    ####################################################################
