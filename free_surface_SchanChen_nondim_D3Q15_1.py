...
#discrete velocity channels for D2Q15
channel_range_14 = 14
channel_range_15 = 15
c = np.array([
    [ 0,  0,  0],  # 0: rest
...
cx = c[:, 0][:, np.newaxis, np.newaxis, np.newaxis]    # (channel_range_15, 1, 1, 1)
cy = c[:, 1][:, np.newaxis, np.newaxis, np.newaxis] 
cz = c[:, 2][:, np.newaxis, np.newaxis, np.newaxis] 

#Inamuro eq(8): constant E & #Krüger: force weights -> expand to D3Q15
# E: main weights
E = np.array([
    (2/9),        # i=0 rest particle
    *((1/9) for _ in range(6)),  # 6 axis-aligned faces
    *((1/72) for _ in range(8))  # 12 edge-aligned edges
])

# H: only rest particle has 1
H = np.array([
    1,  # i=0
] + [0]*channel_range_14)  # i=1..channel_range_15
...
H_exp = H[:, np.newaxis, np.newaxis, np.newaxis]  # (channel_range_15,1,1,1)
F_exp = F[:, np.newaxis, np.newaxis, np.newaxis]  # (channel_range_15,1,1,1)
# Equilibrium weights for h_i
E_exp = E[:, np.newaxis, np.newaxis, np.newaxis]  # Shape (channel_range_15, 1, 1, 1) for broadcasting
neigh_c = c[1:channel_range_15]  # (channel_range_15, 3)
...
def psi(_phi, iteration=None):
    """
    Inamuro eq(11): Compute bulk free-energy density for 3D D3Q15.
...
def p0(_phi):
    """
    Inamuro eq(10): Compute p0 from free-energy for 3D D3Q15.
    _phi: Order parameter (numpy array, shape (Xn+2, Yn+2, Zn+2)).
...
def gradient_p(p):
    """
    Compute pressure gradient for 3D D3Q15 using c_first_derivative with bounce-back at y=0, Yn+1, z=0, Zn+1.
...
def c_first_derivative(lamda, n_dx=1.0, n_dy=1.0, n_dz=1.0):
    """
    D3Q15 gradient using weighted sum.
    """
    ...
    # Weights for i=1 to channel_range_15 (moving particles)
    # Velocity components
    cx = c[1:channel_range_15, 0]
    cy = c[1:channel_range_15, 1]
    cz = c[1:channel_range_15, 2]

    # Shifted neighbors
    shifted = np.stack([
        np.roll(lamda, shift=(c[i+1,0], c[i+1,1], c[i+1,2]), axis=(0,1,2))
        for i in range(channel_range_14)
    ], axis=0)  # (channel_range_15, nx, ny, nz)

    # Weighted sum
    dlamda_dx = scale_x * np.einsum('i,ijkl->jkl', E[1:channel_range_15] * cx, shifted)
    dlamda_dy = scale_y * np.einsum('i,ijkl->jkl', E[1:channel_range_15] * cy, shifted)
    dlamda_dz = scale_z * np.einsum('i,ijkl->jkl', E[1:channel_range_15] * cz, shifted)
...
def c_second_derivative(lamda, n_dx=1.0, n_dy=1.0, n_dz=1.0):
    """
    Vectorized version of eq(13) in Inamuro for 3D D3Q15 - second derivatives.
    ...
    # Lattice speed of sound squared for D3Q15
    scale_x = 1. / (5.* n_dx)
    scale_y = 1. / (5.* n_dy)
    scale_z = 1. / (5.* n_dz)

    # Non-zero velocities (i=1 to channel_range_15) and weights
    neigh_E = E[1:channel_range_15]  # Weights for i=1 to channel_range_15
    
    roll_axes = (0, 1, 2)

    shifted_neighbors = [
        np.roll(lamda, shift=(c[k+1, 0], c[k+1, 1], c[k+1, 2]), axis=roll_axes)
        for k in range(channel_range_14)
    ]
    neighbors_stack = np.stack(shifted_neighbors, axis=0)  # Shape (channel_range_15, nx, ny, nz)

    # Weighted sum over neighbors
    sum_neighbors = np.einsum('h,hijk->ijk', neigh_E, neighbors_stack)

    # Apply formula: scale * (sum - sum_w * lamda)
    d2lamda_dx2 = scale_x * (sum_neighbors - channel_range_14 * lamda)
    d2lamda_dy2 = scale_y * (sum_neighbors - channel_range_14 * lamda)
    d2lamda_dz2 = scale_z * (sum_neighbors - channel_range_14 * lamda)
...
def tau_h(_rho):
    """
    Inamuro eq(23): Compute relaxation time tau_h for 3D D3Q15.
...
def ph(hn, _rho, u_ckl_star, iteration, n_dx=1.0, n_dy=1.0, n_dz=1.0):
    """
    Inamuro eq(22,24): Vectorized evolution for h_i in 3D D3Q15.
    hn: Pressure distribution (shape (channel_range_15, Xn+2, Yn+2, Zn+2)).
    _rho: Density field (shape (Xn+2, Yn+2, Zn+2)).
    ...
    h_eq = E_exp * _p  # Broadcast to (channel_range_15, nx, ny, nz)
...
def gi(_gi, _gi_c, u_ckl, _rho, mu, iteration):
    """
    Vectorized Inamuro eq(3): predicted velocity evolution for gi in two-phase LBM.
    Full batch operations on (channel_range_15, nx, ny, nz) - assumes padded grids (e.g., 202x52x52).
    ...
    _rho_exp = _rho[np.newaxis, :, :, :]  # (1, nx+2, ny+2, nz+2)

    # 6. Viscous term: 3 * E_i * c_a[i] * div_sigma_a / _rho
    dot_term = cx * (div_sigma_x_exp / _rho_exp) + \
               cy * (div_sigma_y_exp / _rho_exp) + \
               cz * (div_sigma_z_exp / _rho_exp)
    viscous_term = 3.0 * E_exp * dot_term * n_dx  # (channel_range_15, nx+2, ny+2, nz+2)
    ...
    streamed = [
        np.roll(_gi_star[i], shift=(c[i, 0], c[i, 1], c[i, 2]), axis=(0, 1, 2)) for i in range(channel_range_15)
    ]
...
def gi_c(u, rho, tau_g, Kg, iteration):
    ...
    E_exp = E[:, np.newaxis, np.newaxis, np.newaxis]  # (channel_range_15,1,1,1)
    
    # term1: E[i] * 1
    term1 = E_exp * ones  # (channel_range_15,nx,ny,nz)
    
    # term2: E[i] * 3 * (c_i . u)
    c_dot_u = np.einsum('ia,axyz->ixyz', c, u)  # (channel_range_15,nx,ny,nz)
    ...
    _Gab = np.einsum('ia,ib,xyzab->ixyz', c, c, Gab_phi)  # (channel_range_15,nx,ny,nz)
    ...
    # Assemble
    _gi_c = term1 + term2 - term3 + term4 + term5 + term6 - term7  # (channel_range_15,nx_pad,ny_pad,nz_pad)
...
def fi(_fi, _fi_c, tau_f):
    """
    Collision and streaming for fi distribution (D3Q15).
    No Python loops anywhere - negative debug uses vectorized argmin/where.
    Assumes globals: RAISE_LESS_THAN_ZERO_ERROR, iteration, c(channel_range_15,2).
    """
    ...
    streamed = [
        np.roll(_fi_star[i], shift=(c[i, 0], c[i, 1], c[i, 2]), axis=(0, 1, 2)) for i in range(channel_range_15)
    ]
...
def c_laplacian(lamda, n_dx=1.0, n_dy=1.0, n_dz=1.0):
    """
    Inamuro eq(13): Compute ∇²λ = ∂²λ/∂x² + ∂²λ/∂y² + ∂²λ/∂z² for 3D D3Q15.
    lamda: Scalar field (shape (Xn+2, Yn+2, Zn+2)).
    n_dx, n_dy, n_dz: Grid spacings.
...
def fi_c(u, Kf, F, _phi, C=0.0):
    ...
    c_dot_u = np.einsum('ia,axyz->ixyz', c, u)  # (channel_range_15,nx,ny) - c_i . u
    term5 = 3.0 * E_exp * _phi * c_dot_u
    
    # term6: E[i] * Kf * _Gab (c^T G c)
    _Gab = np.einsum('ia,ib,xyzab->ixyz', c, c, G)  # (channel_range_15,nx,ny,nz)
    term6 = E_exp * Kf * _Gab
    
    # Assemble _fi_c
    _fi_c = term1 + term2 - term3 - term4 + term5 + term6  # (channel_range_15,nx_pad,ny_pad,nz_pad)
...
def mobility_term(phi, Kf, p0_val, C):
    ...
    # --- build term for all channel_range_15 directions ---
...
def periodic_boundary_conditions(_fi, _gi, Xn, Yn, Zn):
    """
    Apply periodic boundary conditions in x-direction for D3Q15.
    _fi, _gi: Distributions (shape (channel_range_15, nx+2, ny+2, nz+2)).
...
def initialize_phi_line(xn, yn, zn, phi_star_g, phi_star_l, height=None, xi=2.0):
    """
    Initialize _phi along a line at height y in D3Q15, constant along z.
    Args:
...
h0 = np.zeros((channel_range_15,Xn+2, Yn+2, Zn+2),dtype=np.float64)
h = np.zeros((channel_range_15, Xn+2, Yn+2, Zn+2), dtype=np.float64)
_p0 = np.zeros((Xn+2, Yn+2, Zn+2),dtype=np.float64)
_fi_c = np.zeros((channel_range_15,Xn+2, Yn+2, Zn+2),dtype=np.float64)
_fi = np.zeros((channel_range_15,Xn+2, Yn+2, Zn+2),dtype=np.float64)
# Before the main loop
_fi = fi_c(np.zeros_like(u_ckl), Kf, F, _phi)
_gi_c = np.zeros((channel_range_15, Xn+2, Yn+2, Zn+2),dtype=np.float64)
_gi = np.zeros((channel_range_15, Xn+2, Yn+2, Zn+2),dtype=np.float64)
...
while iteration < TOTAL_ITERATIONS:
    ...
    #### MASS CONSERVATION MONITOR — D3Q27 — FULL DOMAIN SUM ####
    total_phi = np.sum(_phi)  # FULL lattice including ALL ghost cells

    if iteration == 0:
        initial_phi = total_phi
        debug_log('MASS', 'Initial total phi (FULL) = %.10f', initial_phi)
    else:
        delta_phi = total_phi - initial_phi
        rel_error = delta_phi / initial_phi if abs(initial_phi) > 1e-8 else 0.0
        debug_log('MASS', 'Iter %d: total phi = %.10f | Δphi = %.1f | rel = %.3e',
                iteration, total_phi, delta_phi, rel_error)

        # Optional: warn if drift is bad
        if abs(rel_error) > 1e-8:
            debug_log('MASS', 'WARNING: Mass drift detected! Check BCs or collision.')
    #################################################    
