...
def m_vector(fc, wall):
    ...
    theta is the contact angle inside the liquid relative to the wall normal
    """
    ...
    if wall == 'bottom':
        ...
        n = np.array([1, 0])   # normal into fluid
        t = np.array([0, 1])   # tangent up
        m = n * np.cos(phi) - t * np.sin(phi)  # note the minus
        return m / np.linalg.norm(m)
    elif wall == 'right':
        n = np.array([-1, 0])  # normal into fluid
        t = np.array([0, 1])   # tangent up
        m = n * np.cos(phi) - t * np.sin(phi)  # minus
        return m / np.linalg.norm(m)
    else:
...
def surfaceTensionForces(iteration, fc, _phi, rho):
    ...
    _zhang_fc = zhang_fc(fc, _phi_n)
    if ADD_METRICS and (iteration == 0 or iteration == (fc.TOTAL_ITERATIONS - 1) or iteration % 100 == 0):
        fcBounds.append((iteration,
                        _zhang_fc[0,1,:].copy(), # left-x
                        _zhang_fc[1,1,:].copy(), # left-y
                        _zhang_fc[0,Xn,:].copy(), # right-x  
                        _zhang_fc[1,Xn,:].copy())) # right-y 
    ...
    zhang_surface_tension_force_new = zhang_fc(fc, _phi)
...
if ADD_METRICS: 
    ...
    iteration, left_x, left_y, right_x, right_y = fcBounds[-1]  # or .pop() if you want to remove
    #plotter.phi_x_axis_plot_2(ax1[3, 1], zhangChemicalPotential_center, inamuroChemicalPotential_center, yc, iteration, f"ChemicalPotential distribution y={yc}", label1, label2)
    #plotter.phi_x_axis_plot_2(
    #            ax2[2, 3], 
    #            series1, series2,
    #            yc=yc, 
    #            iteration=iteration,
    #            title=f"Zhang fc forces y={yc}",
    ...
    plotter.phi_boundary_forces_vertical(
                ...
                yc=yc,
                iteration=iteration,
                title="Capillary forces on vertical boundaries",
                figsize=(7, 9)
            )    
